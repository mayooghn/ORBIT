...
var getExternalMonitoringStatus = (events, now = Date.now()) => {
  const hasRecentExternalEvent = events.some((record) => {
  ...
  return hasRecentExternalEvent ? { state: "ACTIVE", message: "Monitoring is active through the external ingestion pipeline." } : { state: "WAITING", message: "Waiting for new events from the external ingestion pipeline." };
};
...
var friendlyNodeLabel = (nodeId, nodeType) => {
  ...
  if (!typeLabel) return "Digital Twin asset";
...
var relevanceSummary = (result) => {
  ...
  return uniqueSummaryItems([
    ...
    matchedCount ? `${matchedCount} Digital Twin asset${matchedCount === 1 ? "" : "s"} match the event.` : "",
...
var impactSummary = (result) => {
  ...
  return uniqueSummaryItems([
    affectedNodes ? `The Digital Twin identifies ${affectedNodes} affected asset${affectedNodes === 1 ? "" : "s"}.` : "",
  ...
  const loadMonitoring = async () => {
    try {
      ...
      [...events, ...alerts].forEach((record) => {
        const key = record.article?.id || record.detectedAt || `${record.article?.title || "event"}-${record.article?.publishedAt || ""}`;
        if (!byId.has(key)) byId.set(key, record);
      });
      setMonitoredEvents([...byId.values()].sort((left, right) => String(right.detectedAt || "").localeCompare(String(left.detectedAt || ""))));
      const alertsById = /* @__PURE__ */ new Map();
      alerts.forEach((record) => {
        const key = record.article?.id || record.detectedAt || `${record.article?.title || "alert"}-${record.article?.publishedAt || ""}`;
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "space-y-6", children: [
    ...
    result && /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("section", { "aria-labelledby": "geopolitical-assessment-title", className: "rounded-xl border border-[#2A2A2A] bg-[#111111] p-4 sm:p-6", children: [
      ...
      /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "mt-6 space-y-6", children: [
        ...
        /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(ResultSection, { title: "Digital Twin Impact", icon: import_lucide_react2.Network, children: [
          /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(DetailGrid, { items: [
            ["Affected nodes", result.digitalTwinImpact?.affectedNodeIds?.length],
            ["Affected edges", result.digitalTwinImpact?.affectedEdgeIds?.length],
            ["Affected node types", humanizedListOrUnavailable(result.digitalTwinImpact?.affectedNodeTypes)],
...
var MonitoringSection = ({ status, events, alerts, loading, error, onViewDigitalTwin }) => {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("section", { className: "rounded-lg border border-[#222222] bg-[#121212] p-4 sm:p-5", children: [
    /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-[#222222]", children: [
      ...
      /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "flex flex-col items-start sm:items-end gap-1", children: [
        /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(StatusBadge, { level: externalStatus.state === "ACTIVE" ? "AVAILABLE" : "MONITORING", label: externalStatus.state, size: "sm" }),
        /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { className: "text-[10px] text-[#666666] font-mono", children: externalStatus.state === "ACTIVE" ? "External ingestion pipeline" : "No recent external events" })
      ] })
    ] }),
    loading ? /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "py-5 text-xs text-[#888888] flex items-center gap-2", children: [
      ...
      /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "flex flex-wrap gap-x-5 gap-y-1 mt-4 text-xs font-mono text-[#777777]", children: [
        ...
        /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("span", { children: [
          "Last scan: ",
          status?.lastSuccessfulScan ? new Date(status.lastSuccessfulScan).toLocaleString() : "Not available"
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { children: "Refresh: 60s" })
      ] }),
      ...
      events.length > 0 && /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "mt-6 space-y-3", children: [
        ...
        events.slice(0, 6).map((record) => /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(MonitoredEventCard, { record, onViewDigitalTwin }, record.article?.id || record.detectedAt))
      ] }),
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("article", { className: "rounded-lg border border-[#222222] bg-[#0D0D0D] p-4", children: [
    ...
    hasDigitalTwinImpact && /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("details", { className: "mt-4 rounded-md border border-[#222222] bg-[#121212]", children: [
      ...
      /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "grid grid-cols-1 lg:grid-cols-2 gap-4 px-3 pb-3 text-xs", children: [
        /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(NodeIdList, { label: "Affected Digital Twin assets", nodeIds: affectedNodes, nodeTypes: analysis?.digitalTwinImpact?.affectedNodeTypes }),
...
(0, import_node_test.default)("automatic monitoring reports ACTIVE for a recent external webhook event", () => {
  ...
  import_strict.default.equal(status.message, "Monitoring is active through the external ingestion pipeline.");
  import_strict.default.doesNotMatch(status.message, /ORBIT_MONITORING_ENABLED|RSS polling/);
});
(0, import_node_test.default)("automatic monitoring reports WAITING when external events are stale or absent", () => {
  const now = Date.parse("2026-08-23T12:00:00.000Z");
  const status = getExternalMonitoringStatus([
    externalEvent(new Date(now - EXTERNAL_MONITORING_FRESHNESS_MS - 1).toISOString())
  ], now);
  import_strict.default.equal(status.state, "WAITING");
  import_strict.default.equal(status.message, "Waiting for new events from the external ingestion pipeline.");
  import_strict.default.equal(getExternalMonitoringStatus([], now).state, "WAITING");
