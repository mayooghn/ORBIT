...
// tests/geopolitical-supply-chain-relevance.test.ts
...
var analyzeGeopoliticalSupplyChainRelevance = (value2, graph2, classification) => {
  const event = validateGeopoliticalEvent(value2);
  assertGraph(graph2);
  const classified = classifyInput(event, classification);
  const matches = graph2.nodes.map((node) => findNodeMatch(event, node)).filter((match) => match !== void 0);
...
var GeopoliticalSupplyChainRelevanceAnalyzer = class {
  constructor(graph2) {
    this.graph = graph2;
  }
  analyze(value2, classification) {
    return analyzeGeopoliticalSupplyChainRelevance(value2, this.graph, classification);
  }
};

// tests/geopolitical-supply-chain-relevance.test.ts
var processedDir = import_node_path3.default.join(process.cwd(), "data", "processed");
var temporaryDirectory = (0, import_node_fs3.mkdtempSync)(import_node_path3.default.join((0, import_node_os.tmpdir)(), "orbit-geopolitical-relevance-"));
...
var ingestAndClassify = (overrides = {}) => {
...
(0, import_node_test.default)("Hormuz event matches the existing chokepoint", () => {
  ...
  import_strict.default.ok(result.matchedLocations.includes("Strait of Hormuz"));
});
(0, import_node_test.default)("maritime disruption matches existing Hormuz routes and chokepoint", () => {
  ...
  const result = analyzeGeopoliticalSupplyChainRelevance(event, graph, classification);
  import_strict.default.ok(result.matchedNodeIds.includes("shipping-route-persian-gulf-hormuz-arabian-sea"));
  import_strict.default.ok(result.matchedNodeIds.includes("shipping-route-hormuz-india"));
  import_strict.default.ok(result.matchedNodeIds.includes("chokepoint-strait-of-hormuz"));
});
(0, import_node_test.default)("port event matches Vadinar Terminal", () => {
  ...
  const result = analyzeGeopoliticalSupplyChainRelevance(event, graph, classification);
  import_strict.default.ok(result.matchedNodeIds.includes("port-port-42e3af128436239dad1c"));
  import_strict.default.ok(result.matchedNodeTypes.includes("port"));
});
(0, import_node_test.default)("refinery event matches MRPL Mangalore", () => {
  ...
  const result = analyzeGeopoliticalSupplyChainRelevance(event, graph, classification);
  import_strict.default.ok(result.matchedNodeIds.includes("refinery-refinery-2e0d4ad0d99de43e1e73"));
  import_strict.default.ok(result.matchedNodeTypes.includes("refinery"));
});
(0, import_node_test.default)("strategic reserve event matches ISPRL Mangalore", () => {
  ...
  const result = analyzeGeopoliticalSupplyChainRelevance(event, graph, classification);
  import_strict.default.ok(result.matchedNodeIds.includes("strategic-reserve-isprl-mangalore"));
  import_strict.default.ok(result.matchedNodeTypes.includes("strategic_reserve"));
});
(0, import_node_test.default)("country matching finds the existing Saudi Arabia supplier", () => {
  const { event, classification } = ingestAndClassify({ id: "country-event", title: "Saudi Arabia export restriction", description: "A documented crude export restriction was announced.", location: void 0, countriesInvolved: ["Saudi Arabia"], category: "trade_restriction" });
  const result = analyzeGeopoliticalSupplyChainRelevance(event, graph, classification);
  import_strict.default.ok(result.matchedNodeIds.some((nodeId) => graph.nodes.find((node) => node.nodeId === nodeId)?.name === "Saudi Arabia"));
  import_strict.default.ok(result.matchedCountries.includes("Saudi Arabia"));
});
(0, import_node_test.default)("unrelated event returns irrelevant", () => {
  ...
  import_strict.default.deepEqual(result.matchedNodeIds, []);
});
(0, import_node_test.default)("one event can match multiple existing entities", () => {
...
(0, import_node_test.default)("matching is case-insensitive and punctuation-normalized", () => {
...
(0, import_node_test.default)("relevance reasons explain matching and existing relationships", () => {
  ...
  import_strict.default.ok(result.relevanceReasons.some((reason) => reason.includes("location")));
  import_strict.default.ok(result.relevanceReasons.some((reason) => reason.includes("entity name")));
  import_strict.default.ok(result.relevanceReasons.some((reason) => reason.includes("existing graph relationships")));
});
(0, import_node_test.default)("Digital Twin and event inputs remain unchanged", () => {
  ...
  import_strict.default.equal(JSON.stringify(graph), graphBefore);
