...
var createGroqAgentProvider = (overrides = {}) => new GroqService({
  ...overrides,
  apiKey: overrides.apiKey ?? process.env.GROQ_AGENT_API_KEY ?? process.env.GROQ_API_KEY ?? "",
  model: overrides.model ?? process.env.GROQ_AGENT_MODEL ?? process.env.GROQ_MODEL ?? DEFAULT_GROQ_MODEL,
  configurationName: "AGENT"
});
var createGroqNewsProvider = (overrides = {}) => new GroqService({
  ...overrides,
  // NEWS must never fall back to either the AGENT or legacy credential.
