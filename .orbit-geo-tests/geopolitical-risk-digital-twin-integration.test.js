...
// tests/geopolitical-risk-digital-twin-integration.test.ts
...
var insertShippingLanes = (database, processedDir2, rows, sourceIds) => {
...
var insertFacts = (database, processedDir2, sourceIds, periodIds, countryIds, productIds) => {
  const counts = {};
...
var insertQuality = (database, processedDir2, sourceIds) => {
  const counts = {};
...
var insertManualReview = (database, processedDir2, sourceIds) => {
...
// tests/geopolitical-risk-digital-twin-integration.test.ts
var processedDir = import_node_path3.default.join(process.cwd(), "data", "processed");
var temporaryDirectory = (0, import_node_fs3.mkdtempSync)(import_node_path3.default.join((0, import_node_os.tmpdir)(), "orbit-geopolitical-integration-"));
...
(0, import_node_test.default)("relevant Hormuz event matches the Digital Twin and returns impact", () => {
  ...
  import_strict.default.equal(result.relevant, true);
  import_strict.default.ok(result.matchedNodeIds.includes("chokepoint-strait-of-hormuz"));
  import_strict.default.ok(result.impactReasons.some((reason) => reason.includes("chokepoint-strait-of-hormuz")));
});
(0, import_node_test.default)("relevant supplier event matches the existing supplier node", () => {
  const { result } = prepare({ id: "supplier-integration-event", title: "Saudi Arabia export restriction", description: "A documented crude export restriction was announced.", location: void 0, countriesInvolved: ["Saudi Arabia"], category: "trade_restriction" });
  import_strict.default.equal(result.relevant, true);
  import_strict.default.ok(result.matchedNodeIds.some((nodeId) => runtime.stateEngine.getCurrentTwin().nodes.find((node) => node.nodeId === nodeId)?.name === "Saudi Arabia"));
});
(0, import_node_test.default)("port and refinery events match the correct existing entities", () => {
  ...
  import_strict.default.ok(port.result.matchedNodeIds.includes("port-port-42e3af128436239dad1c"));
  import_strict.default.ok(refinery.result.matchedNodeIds.includes("refinery-refinery-2e0d4ad0d99de43e1e73"));
});
(0, import_node_test.default)("impact propagates through existing Digital Twin edges", () => {
  const { result } = prepare({ id: "propagation-integration-event", title: "Saudi Arabia export restriction", description: "A documented crude export restriction was announced.", location: void 0, countriesInvolved: ["Saudi Arabia"], category: "trade_restriction" });
  import_strict.default.ok(result.affectedEdgeIds.includes("relationship-supplier-saudi-arabia-hormuz-route"));
  import_strict.default.ok(result.affectedNodeIds.includes("chokepoint-strait-of-hormuz"));
  import_strict.default.ok(result.affectedNodeIds.includes("shipping-route-hormuz-india"));
  import_strict.default.ok(result.affectedFlow.nodeTotals.some((measurement) => measurement.value === 688e4 && measurement.unit === "barrels_per_day"));
  import_strict.default.ok(result.impactReasons.some((reason) => reason.includes("relationship-supplier-saudi-arabia-hormuz-route")));
});
(0, import_node_test.default)("multiple matched nodes are combined with deduplicated impact", () => {
  ...
  import_strict.default.ok(result.matchedNodeIds.length > 1);
  import_strict.default.equal(new Set(result.affectedNodeIds).size, result.affectedNodeIds.length);
  import_strict.default.equal(new Set(result.affectedEdgeIds).size, result.affectedEdgeIds.length);
  import_strict.default.ok(result.impactReasons.some((reason) => reason.includes("deduplicated")));
});
(0, import_node_test.default)("irrelevant events produce no Digital Twin impact", () => {
  ...
  import_strict.default.equal(result.relevant, false);
...
(0, import_node_test.default)("integration leaves the baseline Digital Twin unchanged", () => {
  const before2 = JSON.stringify(runtime.stateEngine.getCurrentTwin());
  ...
  import_strict.default.equal(JSON.stringify(runtime.stateEngine.getCurrentTwin()), before2);
});
(0, import_node_test.default)("risk level and score are preserved exactly", () => {
  ...
  import_strict.default.equal(result.riskLevel, risk.riskLevel);
  import_strict.default.equal(result.riskScore, risk.riskScore);
});
(0, import_node_test.default)("missing capacity and flow remain unavailable", () => {
  const model = new DigitalTwinGraphModel();
  ...
  const miniRuntime = { stateEngine, impactAnalyzer: new DigitalTwinImpactAnalyzer(stateEngine) };
...
(0, import_node_test.default)("repeated integration is deterministic", () => {
...
(0, import_node_test.default)("integration rejects a matched node that is absent from the graph", () => {
  ...
  import_strict.default.throws(() => integrateGeopoliticalRiskWithDigitalTwin(
    classification,
