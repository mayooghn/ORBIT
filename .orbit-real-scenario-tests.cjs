...
// tests/real-scenario-procurement.test.ts
...
(0, import_node_test.default)("EiaPriceService returns structured benchmark economics", () => {
  const eiaService = new EiaPriceService();
  const saudiEconomics = eiaService.getSupplierEconomics("Saudi Arabia");
  import_strict.default.equal(saudiEconomics.countryName, "Saudi Arabia");
  import_strict.default.match(saudiEconomics.benchmarkName, /Arab Light/i);
  import_strict.default.ok(saudiEconomics.basePriceUsdPerTonne > 0);
  import_strict.default.ok(saudiEconomics.freightCostUsdPerTonne > 0);
  ...
  import_strict.default.ok(saudiEconomics.transitDistanceNm > 0);
  import_strict.default.ok(saudiEconomics.standardTransitDays >= 3);
  import_strict.default.ok(saudiEconomics.riskScore >= 0 && saudiEconomics.riskScore <= 100);
  import_strict.default.ok(saudiEconomics.reliabilityScore > 0 && saudiEconomics.reliabilityScore <= 1);
  const transitDays = eiaService.calculateTransitDays(1450, 13);
  import_strict.default.ok(transitDays >= 4 && transitDays <= 7);
});
(0, import_node_test.default)("RealScenarioProcurementDataProvider resolves real Phase 2 suppliers and routes", () => {
  const provider = new RealScenarioProcurementDataProvider(repository);
  const saudiNode = runtime.stateEngine.getCurrentTwin().nodes.find((n) => n.nodeType === "supplier" && n.name.toLowerCase().includes("saudi"));
  import_strict.default.ok(saudiNode, "Saudi supplier node exists");
  ...
  if (resolution.status === "AVAILABLE") {
    import_strict.default.match(resolution.data.source, /Real Procurement Data Layer/i);
    import_strict.default.ok(resolution.data.suppliers.length > 0, "Suppliers list is populated");
    import_strict.default.ok(resolution.data.routes.length > 0, "Routes list is populated");
    import_strict.default.ok(resolution.data.lanes.length > 0, "Lanes list is populated");
    const hasExcludedSupplier = resolution.data.suppliers.some(
      (s) => s.name.toLowerCase().includes("saudi")
    );
    import_strict.default.equal(hasExcludedSupplier, false, "Affected supplier is excluded from alternatives");
    for (const s of resolution.data.suppliers) {
      import_strict.default.equal(s.capacityUnit, scenario.shortageUnit);
      import_strict.default.ok(s.capacity > 0);
    }
    for (const r of resolution.data.routes) {
      import_strict.default.equal(r.capacityUnit, scenario.shortageUnit);
      import_strict.default.ok(r.capacity > 0);
    }
    for (const lane of resolution.data.lanes) {
      import_strict.default.equal(lane.compatible, true);
      import_strict.default.ok(lane.procurementCostPerUnit > 0);
      import_strict.default.equal(lane.procurementCostUnit, "USD_per_tonne");
      import_strict.default.ok(lane.transitTimeDays >= 1);
      import_strict.default.ok(lane.riskScore >= 1 && lane.riskScore <= 100);
      import_strict.default.ok(lane.reliabilityScore > 0 && lane.reliabilityScore <= 1);
    }
  }
});
(0, import_node_test.default)("RealScenarioProcurementDataProvider returns structured UNAVAILABLE for unverified unit", () => {
  const provider = new RealScenarioProcurementDataProvider(repository);
  ...
  import_strict.default.equal(resolution.status, "UNAVAILABLE");
  if (resolution.status === "UNAVAILABLE") {
    import_strict.default.match(resolution.reason, /unverified/i);
    import_strict.default.match(resolution.source, /Real Procurement Data Provider/i);
  }
});
(0, import_node_test.default)("buildProcurementRequestFromScenario constructs valid GLPK request from Real provider", async () => {
  const provider = new RealScenarioProcurementDataProvider(repository);
  const iraqNode = runtime.stateEngine.getCurrentTwin().nodes.find((n) => n.nodeType === "supplier" && n.name.toLowerCase().includes("iraq"));
  import_strict.default.ok(iraqNode);
  const scenario = scenarioEngine.run(runtime.stateEngine, {
  ...
  import_strict.default.equal(resolution.status, "AVAILABLE");
  import_strict.default.ok(resolution.request);
  if (resolution.request) {
    ...
    if (result.status === "OPTIMAL") {
      import_strict.default.ok(result.totalProcured > 0);
      import_strict.default.ok(result.totalCost > 0);
      import_strict.default.ok(result.supplierAllocations.length > 0);
      import_strict.default.ok(result.routeAllocations.length > 0);
    }
  }
});
var server;
...
(0, import_node_test.default)("POST /api/scenarios/procurement uses Real provider by default", async () => {
  const sourceNode = runtime.stateEngine.getCurrentTwin().nodes.find((n) => n.nodeType === "supplier");
  ...
  import_strict.default.ok(data.status === "OPTIMAL" || data.status === "INFEASIBLE");
  import_strict.default.match(data.source || "", /Real Procurement Data/i);
});
(0, import_node_test.default)("POST /api/scenarios/procurement?dataSource=demo uses Demo provider", async () => {
  const sourceNode = runtime.stateEngine.getCurrentTwin().nodes.find((n) => n.nodeType === "supplier");
  import_strict.default.ok(sourceNode);
  ...
  import_strict.default.equal(response.status, 200);
  const data = await response.json();
  import_strict.default.equal(data.status, "OPTIMAL");
  import_strict.default.match(data.source || "", /Demo procurement data/i);
});
