...
function parseGoogleNewsRss(xml, query = "", retrievedAt = (/* @__PURE__ */ new Date()).toISOString()) {
  return extractRssItems(xml).map((itemXml) => parseItem(itemXml, query, retrievedAt)).filter((article) => article !== null);
}
...
async function fetchGoogleNews(options = {}) {
  ...
  results.forEach((result, index) => {
    ...
    successfulFeeds += 1;
    try {
      for (const article of parseGoogleNewsRss(result.value.xml, query, retrievedAt)) {
...
var toMonitoringArticle = (article) => ({ ...article });
