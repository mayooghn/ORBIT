...
// tests/geopolitical-risk-agent.test.ts
...
var GeopoliticalMonitoringService = class {
  ...
  async runScan() {
    ...
    for (const rawArticle of articlesToProcess) {
      ...
      try {
        const analysis = await this.agent.analyze(articleRequest(article), { explanation: "deterministic" });
...
var temporaryDirectory = (0, import_node_fs4.mkdtempSync)(import_node_path4.default.join((0, import_node_os.tmpdir)(), "orbit-geopolitical-agent-"));
...
var server;
var baseUrl = "";
var validExtractedEvent = (overrides = {}) => ({
  id: "agent-event-1",
...
var MockGroqService = class {
  constructor(extractedEvent, explanation = "The deterministic result identifies the matched energy network and its affected assets.") {
    this.extractedEvent = extractedEvent;
    this.explanation = explanation;
    this.extractionRequests = [];
    this.explanationInputs = [];
  }
  async extractEvent(request) {
...
(0, import_node_test.default)("structured request handling runs the Groq-to-ORBIT pipeline", async () => {
  ...
  import_strict.default.equal(result.request, "What happens if the Strait of Hormuz is disrupted?");
  import_strict.default.equal(result.event.id, "agent-event-1");
  import_strict.default.equal(result.classification.eventId, result.event.id);
  import_strict.default.equal(result.relevance.eventId, result.event.id);
  import_strict.default.equal(result.risk.eventId, result.event.id);
  import_strict.default.equal(result.digitalTwinImpact.eventId, result.event.id);
  import_strict.default.equal(service.extractionRequests.length, 1);
  import_strict.default.equal(service.explanationInputs.length, 1);
});
(0, import_node_test.default)("Groq service abstraction parses structured extraction and explanation", async () => {
  const calls = [];
  const client = {
    chat: {
      completions: {
  ...
  import_strict.default.deepEqual(await service.extractEvent("Hormuz disruption"), validExtractedEvent());
  import_strict.default.equal(await service.explain({}), "Deterministic explanation.");
  import_strict.default.equal(calls.length, 2);
  import_strict.default.equal(calls[0].model, "openai/gpt-oss-20b");
  import_strict.default.equal(calls[0].max_completion_tokens, 220);
  import_strict.default.equal(calls[1].max_completion_tokens, 140);
  import_strict.default.ok(calls[1].messages[1].content.length < 1e3);
  import_strict.default.equal(calls[0].response_format.type, "json_schema");
});
(0, import_node_test.default)("deterministic tools are invoked and supplied to Groq explanation", async () => {
  ...
  import_strict.default.ok(explanationInput);
  import_strict.default.equal(explanationInput.event.id, result.event.id);
  import_strict.default.equal(explanationInput.classification.eventId, result.event.id);
  import_strict.default.equal(explanationInput.relevance.eventId, result.event.id);
  import_strict.default.equal(explanationInput.risk.eventId, result.event.id);
  import_strict.default.equal(explanationInput.digitalTwinImpact.eventId, result.event.id);
  import_strict.default.ok(result.relevance.matchedNodeIds.includes("chokepoint-strait-of-hormuz"));
  ...
  import_strict.default.deepEqual(result.event, event);
  import_strict.default.deepEqual(result.classification, classification);
  import_strict.default.deepEqual(result.relevance, relevance);
  import_strict.default.deepEqual(result.risk, risk);
  import_strict.default.deepEqual(result.digitalTwinImpact, digitalTwinImpact);
});
(0, import_node_test.default)("risk result is preserved exactly from deterministic assessment", async () => {
  const eventPayload = validExtractedEvent({ id: "risk-preserved-event" });
  const result = await new GeopoliticalRiskIntelligenceAgent(runtime, new MockGroqService(eventPayload)).analyze("Assess this event.");
  const event = new GeopoliticalEventIngestionStore().ingest(eventPayload);
  const classification = classifyGeopoliticalEvent(event);
  const relevance = analyzeGeopoliticalSupplyChainRelevance(event, runtime.stateEngine.getCurrentTwin(), classification);
  const expectedRisk = assessGeopoliticalRisk(event, classification, relevance);
  import_strict.default.deepEqual(result.risk, expectedRisk);
});
(0, import_node_test.default)("Digital Twin integration result is preserved exactly", async () => {
  const eventPayload = validExtractedEvent({ id: "twin-preserved-event" });
  const result = await new GeopoliticalRiskIntelligenceAgent(runtime, new MockGroqService(eventPayload)).analyze("Assess network impact.");
  const event = new GeopoliticalEventIngestionStore().ingest(eventPayload);
  const classification = classifyGeopoliticalEvent(event);
  const relevance = analyzeGeopoliticalSupplyChainRelevance(event, runtime.stateEngine.getCurrentTwin(), classification);
  const risk = assessGeopoliticalRisk(event, classification, relevance);
  const expectedImpact = integrateGeopoliticalRiskWithDigitalTwin(classification, relevance, risk, runtime);
  import_strict.default.deepEqual(result.digitalTwinImpact, expectedImpact);
});
(0, import_node_test.default)("irrelevant events return a valid result without Digital Twin impact", async () => {
  const eventPayload = validExtractedEvent({ id: "irrelevant-agent-event", title: "Cultural cooperation agreement", description: "Officials signed a cultural cooperation agreement.", location: "Liechtenstein", countriesInvolved: ["Liechtenstein", "Andorra"], category: "diplomatic_escalation", severity: "critical" });
  const service = new MockGroqService(eventPayload);
  const result = await new GeopoliticalRiskIntelligenceAgent(runtime, service).analyze("Explain this unrelated event.");
  const event = new GeopoliticalEventIngestionStore().ingest(eventPayload);
  const expectedClassification = classifyGeopoliticalEvent(event);
  const expectedRelevance = analyzeGeopoliticalSupplyChainRelevance(event, runtime.stateEngine.getCurrentTwin(), expectedClassification);
  const expectedRisk = assessGeopoliticalRisk(event, expectedClassification, expectedRelevance);
  const expectedDigitalTwinImpact = integrateGeopoliticalRiskWithDigitalTwin(expectedClassification, expectedRelevance, expectedRisk, runtime);
  import_strict.default.equal(service.extractionRequests.length, 1);
  import_strict.default.equal(service.explanationInputs.length, 0);
  import_strict.default.match(result.explanation, /^No Groq explanation was required:/);
  import_strict.default.deepEqual(result.event, event);
  import_strict.default.deepEqual(result.classification, expectedClassification);
  import_strict.default.deepEqual(result.relevance, expectedRelevance);
  import_strict.default.deepEqual(result.risk, expectedRisk);
  import_strict.default.deepEqual(result.digitalTwinImpact, expectedDigitalTwinImpact);
  import_strict.default.equal(result.classification.energyRelevant, false);
  import_strict.default.equal(result.relevance.relevant, false);
  import_strict.default.equal(result.risk.riskScore, 0);
  import_strict.default.equal(result.digitalTwinImpact.relevant, false);
  import_strict.default.deepEqual(result.digitalTwinImpact.affectedNodeIds, []);
});
(0, import_node_test.default)("invalid extracted events are rejected by Phase 4.1 validation", async () => {
  const { analyze } = runAgent({ ...validExtractedEvent(), category: "invalid-category" });
  await import_strict.default.rejects(() => analyze("Malformed event."), /Invalid geopolitical event/);
});
(0, import_node_test.default)("missing Groq API key produces a configuration error", async () => {
  const service = new GroqService({ apiKey: "" });
  await import_strict.default.rejects(() => service.extractEvent("Hormuz disruption"), (error) => error instanceof GroqConfigurationError);
});
(0, import_node_test.default)("invalid Groq structured output produces a clear service error", async () => {
  const client = { chat: { completions: { async create() {
    return { choices: [{ message: { content: "{not valid json" } }] };
  } } } };
  const service = new GroqService({ apiKey: "test-key", client });
  await import_strict.default.rejects(() => service.extractEvent("Hormuz disruption"), (error) => error instanceof GroqServiceError && error.message.includes("invalid structured event JSON"));
});
(0, import_node_test.default)("Groq 429 is surfaced with retry timing and never retried by the service", async () => {
  let calls = 0;
  const client = {
    chat: {
      completions: {
        async create() {
          calls += 1;
          const error = Object.assign(new Error("429 rate_limit_exceeded"), { status: 429, headers: new Headers({ "retry-after": "7" }) });
          throw error;
        }
      }
    }
  ...
  const service = new GroqService({ apiKey: "test-key", client });
  await import_strict.default.rejects(() => service.extractEvent("Hormuz disruption"), (error) => error instanceof GroqRateLimitError && error.retryAfterMs === 7e3 && error.retryAt.length > 0);
  import_strict.default.equal(calls, 1);
});
(0, import_node_test.default)("agent protects deterministic inputs and results from Groq mutation", async () => {
  const eventPayload = validExtractedEvent({ id: "immutable-agent-event" });
  const eventBefore = JSON.stringify(eventPayload);
  const graphBefore = JSON.stringify(runtime.stateEngine.getCurrentTwin());
  const mutatingGroq = {
    async extractEvent() {
      return structuredClone(eventPayload);
    },
    async explain(input) {
      input.risk.riskScore = 0;
      input.digitalTwinImpact.affectedNodeIds.push("fake-node");
      return "Explanation completed.";
    }
  };
  const result = await new GeopoliticalRiskIntelligenceAgent(runtime, mutatingGroq).analyze("Do not mutate this.");
  import_strict.default.equal(JSON.stringify(eventPayload), eventBefore);
  import_strict.default.equal(JSON.stringify(runtime.stateEngine.getCurrentTwin()), graphBefore);
  import_strict.default.notEqual(result.risk.riskScore, 0);
  import_strict.default.equal(result.digitalTwinImpact.affectedNodeIds.includes("fake-node"), false);
});
(0, import_node_test.default)("POST /api/geopolitical-risk/agent follows the API convention", async () => {
  const response = await fetch(`${baseUrl}/api/geopolitical-risk/agent`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ request: "What happens if Hormuz is disrupted?" }) });
  import_strict.default.equal(response.status, 200);
  const body = await response.json();
  import_strict.default.equal(body.status, "AVAILABLE");
  import_strict.default.equal(body.request, "What happens if Hormuz is disrupted?");
  import_strict.default.equal(body.risk.riskScore, 80);
  import_strict.default.deepEqual(body.digitalTwinImpact.affectedNodeIds, []);
});
(0, import_node_test.default)("the agent API returns a clean 429 state without retrying after Groq rate limiting", async () => {
  const rateLimitedApp = createApp(new Phase2Repository(database), runtime, {
    async analyze() {
      throw new GroqRateLimitError(6e4);
    }
  });
  const rateLimitedServer = (0, import_node_http.createServer)(rateLimitedApp);
  await new Promise((resolve) => rateLimitedServer.listen(0, "127.0.0.1", resolve));
  const address = rateLimitedServer.address();
  if (!address || typeof address === "string") throw new Error("Rate-limit test server did not bind.");
  try {
    ...
    const body = await response.json();
    import_strict.default.equal(response.status, 429);
    import_strict.default.equal(body.status, "ERROR");
    import_strict.default.equal(body.code, "GROQ_RATE_LIMITED");
    import_strict.default.ok(body.retryAt);
  } finally {
    await new Promise((resolve) => rateLimitedServer.close(() => resolve()));
  }
});
