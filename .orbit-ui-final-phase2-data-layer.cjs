...
var createGeopoliticalRiskIntelligenceAgent = (runtime, llm = new GroqService()) => new GeopoliticalRiskIntelligenceAgent(runtime, llm);
...
var createApp = (repository2, digitalTwin = createDigitalTwinRuntime(repository2), geopoliticalRiskAgent = createGeopoliticalRiskIntelligenceAgent(digitalTwin), monitoring) => {
  ...
  app.get("/api/phase2/countries", (request, response) => {
    try {
      ...
      response.json(repository2.getCountries(query));
    } catch (error) {
  ...
  app.get("/api/phase2/ports", (request, response) => {
    try {
      const query = { ...listOptions(request), search: queryText(request, "search"), mappingStatus: queryText(request, "mappingStatus") };
      response.json(repository2.getPorts(query));
    } catch (error) {
      handlePhase2Error(response, error);
    }
  });
  app.get("/api/phase2/refineries", (request, response) => {
    try {
      ...
      response.json(repository2.getRefineries(query));
    } catch (error) {
      handlePhase2Error(response, error);
    }
  });
  app.get("/api/phase2/suppliers", (request, response) => {
    try {
      ...
      response.json(repository2.getSuppliers(query));
    } catch (error) {
      handlePhase2Error(response, error);
    }
  });
  app.get("/api/phase2/imports/crude", (request, response) => {
    try {
      ...
      response.json(repository2.getCrudeImports(query));
    } catch (error) {
      handlePhase2Error(response, error);
    }
  });
  app.get("/api/phase2/imports/crude/totals", (request, response) => {
    try {
      const query = { ...listOptions(request), financialYear: queryText(request, "financialYear") };
      response.json(repository2.getCrudeImportTotals(query));
    } catch (error) {
      handlePhase2Error(response, error);
    }
  });
  app.get("/api/phase2/consumption", (request, response) => {
    try {
      ...
      response.json(repository2.getConsumption(query));
    } catch (error) {
      handlePhase2Error(response, error);
    }
  });
  app.get("/api/phase2/global-oil", (request, response) => {
    try {
      ...
      response.json(repository2.getGlobalOil(query));
    } catch (error) {
      handlePhase2Error(response, error);
    }
  });
  app.get("/api/phase2/lanes", (request, response) => {
    try {
      response.json(repository2.getLanes({ ...listOptions(request), category: queryText(request, "category") }));
    } catch (error) {
      handlePhase2Error(response, error);
    }
  });
  app.get("/api/phase2/chokepoints", (request, response) => {
    try {
      response.json(repository2.getChokepoints(listOptions(request)));
    } catch (error) {
      handlePhase2Error(response, error);
    }
  });
  const portActivityHandler = (request, response) => {
    try {
      ...
      response.json(repository2.getPortActivity(query));
    } catch (error) {
      handlePhase2Error(response, error);
    }
  };
  ...
  app.get("/api/phase2/data-quality", (request, response) => {
    try {
      response.json(repository2.getDataQuality({ ...listOptions(request), issueType: queryText(request, "issueType"), severity: queryText(request, "severity"), status: queryText(request, "status") }));
    } catch (error) {
      handlePhase2Error(response, error);
    }
  });
...
async function startServer() {
  ...
  const repository2 = new Phase2Repository(database2);
  const digitalTwin = createDigitalTwinRuntime(repository2);
  ...
  const app = createApp(repository2, digitalTwin, geopoliticalRiskAgent, monitoring);
  monitoring.start();
  const PORT = 3e3;
  ...
  app.listen(PORT, "0.0.0.0", () => {
    ...
    console.log(`[ORBIT Phase 2] Data layer status: ${repository2.getStatus()}`);
  });
}
...
(0, import_node_test.default)("repository queries return real processed values", () => {
  ...
  import_strict.default.equal(refinery.data[0].latitude, null);
