...
var withEnvironment = async (values, callback) => {
  const previous = /* @__PURE__ */ new Map();
  for (const [name, value2] of Object.entries(values)) {
    previous.set(name, process.env[name]);
    if (value2 === void 0) delete process.env[name];
    else process.env[name] = value2;
  }
  try {
    await callback();
  } finally {
    for (const [name, value2] of previous) {
      if (value2 === void 0) delete process.env[name];
      else process.env[name] = value2;
    }
  }
};
var providerInternals = (provider) => provider;
...
(0, import_node_test.default)("Groq service abstraction parses structured extraction and explanation with default 1500 token budget", async () => {
  const calls = [];
  const client = {
    chat: {
      completions: {
  ...
  import_strict.default.equal(calls[0].max_completion_tokens, 1500);
...
(0, import_node_test.default)("EXTRACTION_MAX_COMPLETION_TOKENS environment variable configures completion budget and defaults safely", async () => {
  await withEnvironment({ EXTRACTION_MAX_COMPLETION_TOKENS: "2000" }, async () => {
    import_strict.default.equal(getExtractionMaxCompletionTokens(), 2e3);
  });
  await withEnvironment({ EXTRACTION_MAX_COMPLETION_TOKENS: void 0 }, async () => {
    import_strict.default.equal(getExtractionMaxCompletionTokens(), 1500);
  });
...
(0, import_node_test.default)("the user-facing agent provider uses AGENT credentials and model explicitly", async () => {
  await withEnvironment({
    ...
    const provider = createGroqAgentProvider({ client: {} });
    const internals = providerInternals(provider);
    import_strict.default.equal(internals.apiKey, "agent-secret");
    import_strict.default.equal(internals.model, "agent-model");
    import_strict.default.equal(JSON.stringify(provider).includes("agent-secret"), false);
  });
});
(0, import_node_test.default)("the NEWS provider uses NEWS credentials and model explicitly", async () => {
  await withEnvironment({
    GROQ_AGENT_API_KEY: "agent-secret",
    GROQ_AGENT_MODEL: "agent-model",
    GROQ_NEWS_API_KEY: "news-secret",
    ...
    const provider = createGroqNewsProvider({ client: {} });
    const internals = providerInternals(provider);
    import_strict.default.equal(internals.apiKey, "news-secret");
    import_strict.default.equal(internals.model, "news-model");
    import_strict.default.equal(JSON.stringify(provider).includes("news-secret"), false);
  });
});
(0, import_node_test.default)("the agent provider requires GROQ_AGENT_API_KEY and never falls back to legacy GROQ_API_KEY", async () => {
  await withEnvironment({
    GROQ_AGENT_API_KEY: void 0,
    GROQ_AGENT_MODEL: void 0,
    ...
    await import_strict.default.rejects(
      ...
      (error) => error instanceof GroqConfigurationError && error.message.includes("GROQ_AGENT_API_KEY")
    );
  });
});
(0, import_node_test.default)("the NEWS provider never falls back to AGENT or legacy credentials", async () => {
  await withEnvironment({
    GROQ_AGENT_API_KEY: "agent-only-secret",
    GROQ_AGENT_MODEL: "agent-only-model",
    GROQ_NEWS_API_KEY: void 0,
    GROQ_NEWS_MODEL: void 0,
    GROQ_API_KEY: "legacy-secret",
    GROQ_MODEL: "legacy-model"
  }, async () => {
    await import_strict.default.rejects(
      () => createGroqNewsProvider().extractEvent("Hormuz disruption"),
      (error) => error instanceof GroqConfigurationError && error.message.includes("GROQ_NEWS_API_KEY")
    );
  });
});
...
(0, import_node_test.default)("unmatched events produce deterministic risk score 0 and no Digital Twin impact", async () => {
  ...
  import_strict.default.equal(result.risk.energyRelevant, false);
  import_strict.default.equal(result.risk.riskScore, 0);
  import_strict.default.equal(result.risk.riskLevel, "low");
  import_strict.default.equal(result.digitalTwinImpact.relevant, false);
  import_strict.default.equal(result.digitalTwinImpact.affectedNodeIds.length, 0);
  import_strict.default.equal(result.digitalTwinImpact.affectedEdgeIds.length, 0);
  import_strict.default.match(result.explanation, /classification marked the event as not energy relevant/);
});
(0, import_node_test.default)("invalid extracted events are rejected by validation", async () => {
  const invalidEvent = { ...validExtractedEvent(), severity: "apocalyptic" };
  const { analyze } = runAgent(invalidEvent);
  await import_strict.default.rejects(() => analyze("Analyze invalid event."), (error) => error instanceof Error && error.message.includes("severity"));
});
...
(0, import_node_test.default)("invalid Groq structured output produces a clear service error", async () => {
  ...
  await import_strict.default.rejects(
    ...
    (error) => error instanceof GroqServiceError && error.message.includes("invalid structured event JSON")
  );
});
(0, import_node_test.default)("Groq 429 is surfaced with retry timing and never retried by the service", async () => {
  ...
  error.status = 429;
  error.headers = { "retry-after-ms": "12500" };
  const client = {
    chat: {
      completions: {
        async create() {
  ...
  await import_strict.default.rejects(
    () => service.extractEvent("Hormuz disruption"),
    (rateLimitError) => rateLimitError instanceof GroqRateLimitError && rateLimitError.status === 429 && rateLimitError.retryAfterMs === 12500 && Number.isFinite(Date.parse(rateLimitError.retryAt))
  );
});
(0, import_node_test.default)("agent protects deterministic inputs and results from Groq mutation", async () => {
  ...
  const result = await analyze("What happens if the Strait of Hormuz is disrupted?");
  import_strict.default.equal(result.request, "What happens if the Strait of Hormuz is disrupted?");
  import_strict.default.equal(result.event.title, "Strait of Hormuz disruption");
  import_strict.default.equal(result.risk.riskScore, 87);
});
(0, import_node_test.default)("POST /api/geopolitical-risk/agent follows the API convention", async () => {
  ...
  import_strict.default.equal(body.event.title, "Strait of Hormuz disruption");
});
(0, import_node_test.default)("the agent API returns a clean 429 state without retrying after Groq rate limiting", async () => {
  ...
  rateLimitError.headers = { "retry-after-ms": "5000" };
  const rateLimitedProvider = {
    async extractEvent() {
      throw new GroqRateLimitError(5e3);
    },
    async explain() {
  ...
  const rateLimitedServer = (0, import_node_http.createServer)(app);
  ...
  try {
    ...
    import_strict.default.equal(body.retryAfterMs, 5e3);
  } finally {
