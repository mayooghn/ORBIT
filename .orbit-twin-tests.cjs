...
var OPERATIONAL_STATES, DigitalTwinGraphModel;
...
var fromPhase2_exports = {};
__export(fromPhase2_exports, {
  LIST_A_PORT_IDS: () => LIST_A_PORT_IDS,
  buildDigitalTwinFromPhase2: () => buildDigitalTwinFromPhase2
});
...
var init_fromPhase2 = __esm({
  "src/digitalTwin/fromPhase2.ts"() {
    ...
    buildDigitalTwinFromPhase2 = (repository) => {
...
var getTestProcessedDir = () => {
  const upperData = import_node_path3.default.join(process.cwd(), "Data", "processed");
  if ((0, import_node_fs3.existsSync)(upperData)) return upperData;
  return import_node_path3.default.join(process.cwd(), "data", "processed");
};
...
(0, import_node_test.default)("retaining Digital Twin nodes removes orphaned edges and connections", () => {
  const model = new DigitalTwinGraphModel();
  model.addNode({ nodeId: "supplier-1", nodeType: "supplier", name: "Supplier", operationalState: "operational", stateSource: "BASELINE", sourceReferences: [] });
  ...
  import_strict.default.deepEqual(model.getNodes().map((node) => node.nodeId), ["route-1"]);
  import_strict.default.deepEqual(model.getNodes()[0]?.connectedNodeIds, []);
  import_strict.default.deepEqual(model.getEdges(), []);
});
(0, import_node_test.default)("Digital Twin adapter loads real Phase 2 nodes without inventing relationships", () => {
  ...
  try {
    ...
    import_strict.default.equal(graph.nodes.filter((node) => node.nodeType === "port").length, 13);
    ...
    import_strict.default.equal(graph.nodes.filter((node) => node.nodeType === "supplier").length, 52);
    ...
    const capacityRefineries = graph.nodes.filter((node) => node.nodeType === "refinery" && node.capacity !== void 0);
    import_strict.default.equal(capacityRefineries.length, 24);
    import_strict.default.ok(capacityRefineries.every((node) => node.nodeType === "refinery"));
    import_strict.default.equal(graph.nodes.find((node) => node.name === "CPCL, Cauvery Basin*")?.capacity?.value, 0);
    import_strict.default.equal(graph.nodes.some((node) => node.name === "Azhikal (Azhikkal)"), false);
    import_strict.default.ok(graph.nodes.some((node) => node.name === "Kochi (Cochin)"));
    import_strict.default.ok(graph.nodes.some((node) => node.name === "Iran"));
    import_strict.default.ok(capacityRefineries.every((node) => node.sourceReferences.some((reference) => reference.table === "refineries")));
    import_strict.default.equal(graph.nodes.filter((node) => node.nodeType === "supplier" && node.capacity !== void 0).length, 0);
    import_strict.default.equal(graph.nodes.filter((node) => node.nodeType === "port" && node.capacity !== void 0).length, 0);
    import_strict.default.equal(graph.nodes.filter((node) => node.nodeType === "shipping_route" && node.capacity !== void 0).length, 0);
    import_strict.default.equal(graph.nodes.filter((node) => node.nodeType === "strategic_reserve" && node.capacity !== void 0).length, 3);
    import_strict.default.equal(graph.nodes.filter((node) => node.nodeType === "chokepoint" && node.capacity !== void 0).length, 0);
    const iran = graph.nodes.find((node) => node.nodeType === "supplier" && node.name === "Iran");
    import_strict.default.equal(iran?.currentFlow?.value, 2445e3);
    import_strict.default.equal(iran?.currentFlow?.unit, "barrels_per_day");
    const kochi = graph.nodes.find((node) => node.name === "Kochi (Cochin)");
    import_strict.default.equal(kochi?.currentFlow, void 0);
    import_strict.default.equal(kochi?.metadata.currentFlowUnitStatus, null);
    const { LIST_A_PORT_IDS: LIST_A_PORT_IDS2 } = (init_fromPhase2(), __toCommonJS(fromPhase2_exports));
    import_strict.default.ok(graph.nodes.every(
      (node) => node.connectedNodeIds.length > 0 || node.metadata.sourceBackedOperationalData === true || LIST_A_PORT_IDS2.has(node.nodeId) || node.nodeType === "shipping_route" && node.metadata.geometry !== null
    ));
    import_strict.default.equal(graph.nodes.some((node) => node.nodeId.startsWith("shipping-route-shipping-lane-")), true);
...
(0, import_node_test.default)("Digital Twin disruption analysis reports connected and downstream impact", () => {
  ...
  import_strict.default.deepEqual(result.affectedFlow.nodeTotals, [{ value: 150, unit: "tonnes_per_day" }]);
  import_strict.default.deepEqual(result.affectedFlow.edgeTotals, [{ value: 95, unit: "tonnes_per_day" }]);
});
(0, import_node_test.default)("Digital Twin impact preserves verified direct flow, including zero", () => {
  const model = new DigitalTwinGraphModel();
  ...
  import_strict.default.deepEqual(result.affectedFlow.nodeTotals, [{ value: 0, unit: "barrels_per_day" }]);
});
...
(0, import_node_test.default)("Digital Twin refinery nodes retain real SQLite latitude and longitude coordinates", () => {
  const temporaryDirectory = (0, import_node_fs3.mkdtempSync)(import_node_path3.default.join((0, import_node_os.tmpdir)(), "orbit-digital-twin-coords-"));
  const databasePath = import_node_path3.default.join(temporaryDirectory, "phase2.sqlite");
  const processedDir = getTestProcessedDir();
  let database = openPhase2Database({ dbPath: databasePath });
  try {
    ...
    for (const ref of refineries) {
      import_strict.default.equal(typeof ref.metadata.latitude, "number", `Refinery ${ref.nodeId} missing latitude`);
      import_strict.default.equal(typeof ref.metadata.longitude, "number", `Refinery ${ref.nodeId} missing longitude`);
      import_strict.default.ok(isFinite(ref.metadata.latitude));
      import_strict.default.ok(isFinite(ref.metadata.longitude));
    }
