...
var PHASE2_SCHEMA_SQL, PHASE2_DATA_TABLES;
...
var import_node_fs, import_node_path, import_node_sqlite, defaultPhase2DbPath, openPhase2Database;
var init_database = __esm({
  "src/dataLayer/database.ts"() {
    ...
    init_schema();
    defaultPhase2DbPath = () => process.env.ORBIT_DB_PATH || import_node_path.default.join(process.cwd(), "data", "orbit.db");
    openPhase2Database = (options = {}) => {
...
var import_node_crypto, import_node_fs2, import_node_path2, getProcessedDir, DEFAULT_PROCESSED_DIR, stableId, value, nullable, numberValue, requiredNumber, parseCsv, readCsv, readJson, runStatement, allRows, clearData, insertSourceManifest, insertUnits, insertFinancialPeriods, insertProducts, insertCountries, insertPorts, insertRefineries, insertStrategicReserves, insertShippingLanes, insertFacts, insertQuality, insertManualReview, insertRelationshipStatuses, importPhase2Data;
var init_importer = __esm({
  "src/dataLayer/importer.ts"() {
    ...
    DEFAULT_PROCESSED_DIR = getProcessedDir();
    ...
    runStatement = (database2, sql, parameters = []) => {
      database2.prepare(sql).run(...parameters);
    };
    allRows = (database2, table) => database2.prepare(`SELECT * FROM ${table}`).all();
    clearData = (database2) => {
      for (const table of PHASE2_DATA_TABLES) database2.exec(`DELETE FROM ${table}`);
    };
    insertSourceManifest = (database2, rows) => {
  ...
  `);
    ...
    insertFinancialPeriods = (database2, rows) => {
      const ids = /* @__PURE__ */ new Map();
    ...
    insertProducts = (database2, productRows, aliasRows, sourceIds) => {
      const ids = /* @__PURE__ */ new Map();
    ...
    insertCountries = (database2, countryRows, aliasRows, sourceIds) => {
      const ids = /* @__PURE__ */ new Map();
    ...
    insertPorts = (database2, portRows, mappingRows, sourceIds, countryIds) => {
    ...
    insertRefineries = (database2, rows, sourceIds) => {
  ...
  `);
    ...
    insertShippingLanes = (database2, processedDir, rows, sourceIds) => {
    ...
    insertFacts = (database2, processedDir, sourceIds, periodIds, countryIds, productIds) => {
      const counts = {};
    ...
    insertQuality = (database2, processedDir, sourceIds) => {
      const counts = {};
    ...
    insertManualReview = (database2, processedDir, sourceIds) => {
...
var import_node_crypto2, pageValues, pagedQuery, containsFilter, exactFilter, where, Phase2Repository;
var init_repository = __esm({
  "src/dataLayer/repository.ts"() {
    ...
    pagedQuery = (database2, selectSql, countSql, whereSql, parameters, options) => {
    ...
    containsFilter = (field, value2, clauses, parameters) => {
      if (value2?.trim()) {
        clauses.push(`${field} LIKE ? COLLATE NOCASE`);
        parameters.push(`%${value2.trim()}%`);
      }
    };
    exactFilter = (field, value2, clauses, parameters) => {
      if (value2?.trim()) {
        clauses.push(`${field} = ?`);
        parameters.push(value2.trim());
      }
...
var DIGITAL_TWIN_NODE_TYPES, OPERATIONAL_STATES, DigitalTwinGraphModel;
var init_model = __esm({
  "src/digitalTwin/model.ts"() {
    DIGITAL_TWIN_NODE_TYPES = [
      "supplier",
      "port",
      "refinery",
      "strategic_reserve",
    ...
    OPERATIONAL_STATES = ["operational", "reduced", "disrupted", "blocked"];
    DigitalTwinGraphModel = class {
      constructor() {
...
var EIA_HORMUZ_URL, ISPRL_ANNUAL_REPORT_URL, PPAC_REFINERY_URL, IOCL_CRUDE_PIPELINES_URL, IOCL_HALDIA_PORT_URL, RIL_SIKKA_PORT_URL, VISAKHAPATNAM_PORT_HPCL_URL, NEWS_ON_AIR_MUMBAI_HORMUZ_URL, IOCL_VADINAR_IRAQ_CRUDE_URL, EIA_ORGANIZATION, ISPRL_ORGANIZATION, PPAC_ORGANIZATION, IOCL_ORGANIZATION, RIL_ORGANIZATION, VISAKHAPATNAM_PORT_ORGANIZATION, NEWS_ON_AIR_ORGANIZATION, externalReference, PORT_REFINERY_RELATIONSHIPS, PHASE_37_NODES, PORT_REFINERY_COVERAGE_RELATIONSHIPS, ROUTE_PORT_RELATIONSHIPS, addNodeIfMissing, addEdgeIfSupported, findSupplierNodeId, addSupplierRouteEdges, addRouteAndReserveEdges, enrichDigitalTwinRelationships;
var init_relationships = __esm({
  "src/digitalTwin/relationships.ts"() {
    EIA_HORMUZ_URL = "https://www.eia.gov/international/analysis/special-topics/World_Oil_Transit_Chokepoints";
    ISPRL_ANNUAL_REPORT_URL = "https://isprlindia.com/downloads/annual-reports/Annual_Report_Final_2025_Revised_English.pdf";
    PPAC_REFINERY_URL = "https://ppac.gov.in/infrastructure/installed-refinery-capacity";
    IOCL_CRUDE_PIPELINES_URL = "https://iocl.com/crude-oil-pipelines";
    IOCL_HALDIA_PORT_URL = "https://ioclfiles.iocl.com/Refineries_Technology_with_ecology/files/basic-html/page127.html";
    RIL_SIKKA_PORT_URL = "https://www.ril.com/ar2016-17/pdf/RIL-Integrated-AR-2016-17.pdf";
    VISAKHAPATNAM_PORT_HPCL_URL = "https://vizagport.com/wp-content/uploads/2018/07/BPofVPT.pdf";
    NEWS_ON_AIR_MUMBAI_HORMUZ_URL = "https://newsonair.gov.in/oil-tanker-carrying-crude-oil-reaches-mumbai-after-transiting-strait-of-hormuz/";
    IOCL_VADINAR_IRAQ_CRUDE_URL = "https://www.iocl.com/NewsDetails/59337";
    EIA_ORGANIZATION = "U.S. Energy Information Administration";
    ISPRL_ORGANIZATION = "Indian Strategic Petroleum Reserves Limited";
    PPAC_ORGANIZATION = "Petroleum Planning & Analysis Cell, Government of India";
    IOCL_ORGANIZATION = "Indian Oil Corporation Limited";
    RIL_ORGANIZATION = "Reliance Industries Limited";
    VISAKHAPATNAM_PORT_ORGANIZATION = "Visakhapatnam Port Authority";
    NEWS_ON_AIR_ORGANIZATION = "Akashvani / News On AIR, Government of India";
    externalReference = (id) => ({ table: "external_source", id });
    PORT_REFINERY_RELATIONSHIPS = [
      {
    ...
    PORT_REFINERY_COVERAGE_RELATIONSHIPS = [
      {
    ...
    addNodeIfMissing = (model, input) => {
      if (!model.getNode(input.nodeId)) model.addNode(input);
    };
    addEdgeIfSupported = (model, input) => {
    ...
    findSupplierNodeId = (model, supplierName) => model.getNodes().find((node) => node.nodeType === "supplier" && node.name.toLowerCase() === supplierName.toLowerCase())?.nodeId;
...
var import_node_crypto3, BASELINE_STATE, LIST_A_PORT_IDS, text, number, stableIdentity, sourceReference, addNode, buildDigitalTwinFromPhase2;
var init_fromPhase2 = __esm({
  "src/digitalTwin/fromPhase2.ts"() {
    import_node_crypto3 = require("node:crypto");
    init_model();
    init_relationships();
    BASELINE_STATE = "operational";
    LIST_A_PORT_IDS = /* @__PURE__ */ new Set([
    ...
    stableIdentity = (value2) => (0, import_node_crypto3.createHash)("sha256").update(value2, "utf8").digest("hex").slice(0, 20);
    sourceReference = (table, id) => ({ table, id });
    addNode = (model, input) => {
      model.addNode(input);
    };
    buildDigitalTwinFromPhase2 = (repository2) => {
...
var summarize, measurementSummary, sortedEdges, DigitalTwinImpactAnalyzer;
var init_impact = __esm({
  "src/digitalTwin/impact.ts"() {
    ...
    measurementSummary = (nodes, edges, field) => ({
      nodeTotals: summarize(nodes.map((node) => node[field])),
      edgeTotals: summarize(edges.map((edge) => edge[field]))
    });
    sortedEdges = (graph) => [...graph.edges].sort((left, right) => left.edgeId.localeCompare(right.edgeId));
    DigitalTwinImpactAnalyzer = class {
...
var isOperationalState, cloneNode, cloneGraph, DigitalTwinStateEngine, cloneGraphWithoutStateMutation;
var init_state = __esm({
  "src/digitalTwin/state.ts"() {
    init_model();
    isOperationalState = (value2) => typeof value2 === "string" && OPERATIONAL_STATES.includes(value2);
    ...
    cloneGraphWithoutStateMutation = (graph) => ({
      modelVersion: graph.modelVersion,
      nodes: graph.nodes.map((node) => ({
        ...node,
        connectedNodeIds: [...node.connectedNodeIds],
...
var createDigitalTwinRuntime;
var init_runtime = __esm({
  "src/digitalTwin/runtime.ts"() {
    init_fromPhase2();
    init_impact();
    init_state();
    createDigitalTwinRuntime = (repository2) => {
...
var RECOVERY_MODEL_DESCRIPTION, ALTERNATIVE_CAPACITY_UNAVAILABLE_SOURCE, clampPercent, calculateSupplyLoss, RECOVERY_WINDOW_MULTIPLIERS, calculateRecoveryDays, buildRecoveryTimeline, toScenarioImpacts, getIdsByType, resolveAlternativeCapacity, ScenarioEngine;
var init_scenario_engine = __esm({
  "src/scenarios/scenario-engine.ts"() {
    init_impact();
    RECOVERY_MODEL_DESCRIPTION = "No source-backed recovery rate is available. Recovery is modeled as a deterministic linear return from the disrupted capacity level to 100% over a severity-scaled recovery window.";
    ALTERNATIVE_CAPACITY_UNAVAILABLE_SOURCE = "unavailable: no verified, unit-compatible spare capacity is available in the existing Phase 2/Digital Twin data.";
    clampPercent = (value2) => Math.max(0, Math.min(100, value2));
    calculateSupplyLoss = (baseline, durationDays, capacityReductionPercent) => {
    ...
    RECOVERY_WINDOW_MULTIPLIERS = {
      LOW: 0.5,
      MEDIUM: 0.75,
      HIGH: 1,
      CRITICAL: 1.5
    };
    calculateRecoveryDays = (durationDays, severity) => {
    ...
    buildRecoveryTimeline = (durationDays, recoveryDays, capacityReductionPercent) => {
      const timeline = [];
    ...
    toScenarioImpacts = (impactResult) => {
    ...
    getIdsByType = (graph, nodeIds, type) => graph.nodes.filter(
      (node) => node.nodeType === type && nodeIds.includes(node.nodeId)
    ).map((node) => node.nodeId).sort();
    resolveAlternativeCapacity = (assessment, grossSupplyLossUnit) => {
    ...
    ScenarioEngine = class {
      constructor(baselineProvider2) {
        this.baselineProvider = baselineProvider2;
      }
...
var HORMUZ_NODE_ID, HORMUZ_PORT_NAME, SqliteScenarioBaselineProvider;
var init_sqlite_baseline_provider = __esm({
  "src/scenarios/sqlite-baseline-provider.ts"() {
    HORMUZ_NODE_ID = "chokepoint-strait-of-hormuz";
    HORMUZ_PORT_NAME = "Jawaharlal Nehru Port (Nhava Shiva)";
    SqliteScenarioBaselineProvider = class {
...
var INPUT_FIELDS, validateStrategicReserveInput, optimizeStrategicReserve;
var init_optimizer = __esm({
  "src/reserves/optimizer.ts"() {
    ...
    validateStrategicReserveInput = (value2) => {
    ...
    optimizeStrategicReserve = (input) => {
...
var import_node_crypto4, ENERGY_MONITORING_QUERIES, GOOGLE_NEWS_QUERIES, GOOGLE_NEWS_RSS_URL, REQUEST_TIMEOUT_MS, ingestionStatus, normalizedStoryTitle, fetchFeed;
var init_googleNews = __esm({
  "src/services/dataIngestion/googleNews.ts"() {
    import_node_crypto4 = require("node:crypto");
    ENERGY_MONITORING_QUERIES = [
    ...
    GOOGLE_NEWS_QUERIES = ENERGY_MONITORING_QUERIES;
    GOOGLE_NEWS_RSS_URL = "https://news.google.com/rss/search";
    REQUEST_TIMEOUT_MS = 1e4;
    ingestionStatus = "NOT_CONNECTED";
    normalizedStoryTitle = (title) => title.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
    fetchFeed = async (query) => fetchRssUrl(buildFeedUrl(query));
  }
});

// src/geopoliticalEvents/model.ts
var GEOPOLITICAL_EVENT_CATEGORIES, GEOPOLITICAL_EVENT_SEVERITIES, GeopoliticalEventValidationError, isRecord, requiredText, optionalText, validateTimestamp, validateSourceUrl, validateCountries, validateEnum, validateGeopoliticalEvent;
var init_model2 = __esm({
  "src/geopoliticalEvents/model.ts"() {
    ...
    GEOPOLITICAL_EVENT_SEVERITIES = ["low", "medium", "high", "critical"];
    GeopoliticalEventValidationError = class extends Error {
      constructor(message) {
        super(`Invalid geopolitical event: ${message}`);
        this.name = "GeopoliticalEventValidationError";
      }
    };
    isRecord = (value2) => typeof value2 === "object" && value2 !== null && !Array.isArray(value2);
    requiredText = (value2, field) => {
      if (typeof value2 !== "string" || !value2.trim()) {
        throw new GeopoliticalEventValidationError(`${field} is required.`);
      }
      return value2.trim();
    };
    optionalText = (value2, field) => {
      if (value2 === void 0 || value2 === null) return void 0;
      return requiredText(value2, field);
    };
    validateTimestamp = (value2) => {
    ...
    validateEnum = (value2, field, values) => {
    ...
    validateGeopoliticalEvent = (value2) => {
...
var COUNTRY_REGIONS, LOCATION_REGIONS, ENERGY_TERMS, ENERGY_LOCATIONS, compactCountry, classifyRegion, classifyEnergyRelevance, classifyGeopoliticalEvent;
var init_classification = __esm({
  "src/geopoliticalEvents/classification.ts"() {
    ...
    ENERGY_TERMS = /\b(?:energy|crude|oil|petroleum|refinery|pipeline|tanker|lng|lpg|natural gas|fuel|power plant|strategic reserve|oil terminal)\b/i;
    ENERGY_LOCATIONS = /strait of hormuz|persian gulf|gulf of oman|arabian sea|indian ocean|gulf of kutch|strait of malacca|red sea|suez/i;
    compactCountry = (country) => country.toLowerCase().replace(/[^a-z]/g, "");
    classifyRegion = (event) => {
    ...
    classifyGeopoliticalEvent = (value2) => {
...
var RISK_LEVELS, NODE_TYPES, isRecord2, isStringArray, uniqueSorted, validateClassification, validateRelevance, validateRisk, assertMatchingInputs, emptyMeasurementSummary, summarize2, measurementSummary2, assertGraphNodesExist, irrelevantResult, integrateGeopoliticalRiskWithDigitalTwin;
var init_digitalTwinIntegration = __esm({
  "src/geopoliticalEvents/digitalTwinIntegration.ts"() {
    init_impact();
    init_state();
    RISK_LEVELS = ["low", "medium", "high", "critical"];
    NODE_TYPES = ["supplier", "port", "refinery", "strategic_reserve", "shipping_route", "chokepoint"];
    isRecord2 = (value2) => typeof value2 === "object" && value2 !== null && !Array.isArray(value2);
    isStringArray = (value2) => Array.isArray(value2) && value2.every((item) => typeof item === "string");
    uniqueSorted = (values) => [...new Set(values)].sort((left, right) => left.localeCompare(right));
    validateClassification = (value2) => {
    ...
    emptyMeasurementSummary = () => ({ nodeTotals: [], edgeTotals: [] });
    summarize2 = (measurements) => {
      const totals = /* @__PURE__ */ new Map();
      for (const measurement of measurements) {
        if (!measurement) continue;
        totals.set(measurement.unit, (totals.get(measurement.unit) || 0) + measurement.value);
    ...
    irrelevantResult = (classification, relevance, risk) => {
    ...
    integrateGeopoliticalRiskWithDigitalTwin = (classificationValue, relevanceValue, riskValue, runtime2) => {
...
var DuplicateGeopoliticalEventError, cloneEvent, GeopoliticalEventIngestionStore;
var init_ingestion = __esm({
  "src/geopoliticalEvents/ingestion.ts"() {
    init_model2();
    DuplicateGeopoliticalEventError = class extends Error {
      constructor(eventId) {
    ...
    GeopoliticalEventIngestionStore = class {
      constructor() {
...
var normalize, phraseMatches, metadataText, explicitCountryText, nodeText, findNodeMatch, assertGraph, classifyInput, analyzeGeopoliticalSupplyChainRelevance;
var init_relevance = __esm({
  "src/geopoliticalEvents/relevance.ts"() {
    init_classification();
    init_model2();
    normalize = (value2) => value2.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim().replace(/\s+/g, " ");
    phraseMatches = (phrase, text2) => {
      const normalizedPhrase = normalize(phrase);
      const normalizedText = normalize(text2);
      return normalizedPhrase.length >= 3 && normalizedText.includes(normalizedPhrase);
    };
    metadataText = (node) => Object.values(node.metadata).filter((value2) => typeof value2 === "string" && value2.trim().length > 0).join(" ");
    explicitCountryText = (node) => [
      node.name,
    ...
    nodeText = (node) => [node.name, node.description || "", metadataText(node)].join(" ");
    findNodeMatch = (event, node) => {
    ...
    classifyInput = (event, classification) => {
    ...
    analyzeGeopoliticalSupplyChainRelevance = (value2, graph, classification) => {
      const event = validateGeopoliticalEvent(value2);
      assertGraph(graph);
...
var SEVERITY_POINTS, NODE_TYPE_POINTS, NODE_TYPE_LABELS, riskLevelForScore, isRecord3, validateClassification2, validateRelevance2, addFactor, assessGeopoliticalRisk;
var init_risk = __esm({
  "src/geopoliticalEvents/risk.ts"() {
    init_classification();
    init_model2();
    ...
    isRecord3 = (value2) => typeof value2 === "object" && value2 !== null && !Array.isArray(value2);
    validateClassification2 = (event, value2) => {
    ...
    validateRelevance2 = (event, value2) => {
    ...
    addFactor = (factors, name, points, explanation) => {
      if (points > 0) factors.push({ name, points, explanation });
    };
    assessGeopoliticalRisk = (eventValue, classificationValue, relevanceValue) => {
...
var import_groq_sdk, GroqConfigurationError, GroqServiceError, GroqRateLimitError, DEFAULT_GROQ_MODEL, geopoliticalEventSchema, extractionSystemPrompt, explanationSystemPrompt, DEFAULT_RATE_LIMIT_RETRY_MS, getExtractionMaxCompletionTokens, getExplanationMaxCompletionTokens, headerValue, retryAfterMsFor, isRateLimitError, responseText, compactExplanationInput, GroqService, createGroqAgentProvider, createGroqNewsProvider;
var init_groq = __esm({
  "src/geopoliticalEvents/groq.ts"() {
    import_groq_sdk = __toESM(require("groq-sdk"), 1);
    GroqConfigurationError = class extends Error {
    ...
    GroqRateLimitError = class extends GroqServiceError {
    ...
    extractionSystemPrompt = "Return one JSON ORBIT event matching the schema. Extract only event facts; do not calculate risk or invent Digital Twin assets. For hypothetical requests, use the request as source and current time as timestamp.";
    explanationSystemPrompt = "Write 2 concise sentences explaining the supplied deterministic ORBIT result. Focus on the event impact, risk level, and affected supply chain assets. Do NOT include mathematical point breakdowns or individual factor scores (such as event severity (20), energy relevance (10), etc.). Use only its values; do not recalculate risk or invent assets, relationships, capacities, or flows.";
    DEFAULT_RATE_LIMIT_RETRY_MS = 24 * 60 * 60 * 1e3;
    getExtractionMaxCompletionTokens = () => {
    ...
    getExplanationMaxCompletionTokens = () => {
    ...
    headerValue = (error, name) => {
      if (!error || typeof error !== "object") return void 0;
      const headers = error.headers;
      if (headers && typeof headers.get === "function") {
        const value2 = headers.get(name);
        return value2 || void 0;
      }
      if (headers && typeof headers === "object") {
        const record = headers;
        const value2 = record[name] ?? record[name.toLowerCase()];
        return typeof value2 === "string" ? value2 : void 0;
      }
      return void 0;
    };
    retryAfterMsFor = (error) => {
    ...
    createGroqAgentProvider = (overrides = {}) => new GroqService({
      ...overrides,
    ...
    createGroqNewsProvider = (overrides = {}) => new GroqService({
      ...overrides,
...
var clone, isEnergySupplyChainRelevant, deterministicExplanation, deterministicRelevantExplanation, analyzeEventDeterministically, analyzeGeopoliticalEventDeterministically, GeopoliticalRiskIntelligenceAgent, createGeopoliticalRiskIntelligenceAgent;
var init_agent = __esm({
  "src/geopoliticalEvents/agent.ts"() {
    init_classification();
    init_digitalTwinIntegration();
    init_ingestion();
    init_relevance();
    init_risk();
    init_groq();
    clone = (value2) => structuredClone(value2);
    isEnergySupplyChainRelevant = (classification, relevance, risk) => classification.energyRelevant && relevance.relevant && risk.energyRelevant;
    ...
    analyzeEventDeterministically = (eventValue, runtime2) => {
    ...
    analyzeGeopoliticalEventDeterministically = (request, eventValue, runtime2) => {
    ...
    GeopoliticalRiskIntelligenceAgent = class {
      ...
      async analyze(request, options = {}) {
        ...
        try {
          ...
          return analyzeGeopoliticalEventDeterministically(normalizedRequest, void 0, this.runtime);
        }
    ...
    createGeopoliticalRiskIntelligenceAgent = (runtime2, llm = createGroqAgentProvider()) => new GeopoliticalRiskIntelligenceAgent(runtime2, llm);
  }
});

// src/geopoliticalEvents/deduplication.ts
var STOP_WORDS, SYNONYMS, ACTION_TERMS, normalizeWord, stripSourceSuffix, normalizedDedupTokens, buildArticleFingerprint, buildEventFingerprint, asSet, jaccardSimilarity, areEventDatesWithinWindow, overlappingAnchors, areLikelySameEvent;
var init_deduplication = __esm({
  "src/geopoliticalEvents/deduplication.ts"() {
    ...
    stripSourceSuffix = (title, source) => {
    ...
    normalizedDedupTokens = (value2, source) => {
      const normalized = stripSourceSuffix(value2.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase(), source).replace(/[^a-z0-9]+/g, " ").trim();
      const tokens = normalized.split(/\s+/).map(normalizeWord).filter((token) => token.length >= 3 && !STOP_WORDS.has(token));
      return [...new Set(tokens)].sort();
    };
    buildArticleFingerprint = (article) => normalizedDedupTokens(`${article.title} ${article.description || ""}`, article.source).join("|");
...
var import_node_crypto5, HIGH_CONFIDENCE_THRESHOLD, ENERGY_PATTERN, ASSET_PATTERN, ENERGY_EXPORT_PATTERN, EVENT_RULES, KNOWN_REGIONS, COUNTRY_ALIASES, normalizeText, phraseMatches2, textValue, articleText, validHttpUrl, normalizedTimestamp, stableEventId, sourceCountryNames, aliasesForCountry, buildCountryCandidates, nodeAliases, findNodeMatches, findRegionMatches, findCountries, matchingRule, severityFor, matchedEntityKey, uniqueEntities, confidenceFor, categoryForTradeRule, locationFor, extractionResult, extractDeterministicGeopoliticalEvent;
var init_deterministicExtractor = __esm({
  "src/geopoliticalEvents/deterministicExtractor.ts"() {
    import_node_crypto5 = require("node:crypto");
    init_googleNews();
    init_model2();
    HIGH_CONFIDENCE_THRESHOLD = 0.85;
    ENERGY_PATTERN = /\b(?:energy|crude|oil|petroleum|refiner(?:y|ies)|pipeline(?:s)?|tanker(?:s)?|lng|lpg|natural gas|fuel|power plant|strategic reserve|oil terminal)\b/i;
    ASSET_PATTERN = /\b(?:refiner(?:y|ies)|pipeline(?:s)?|terminal(?:s)?|tanker(?:s)?|shipping|maritime|chokepoint(?:s)?|strait(?:s)?|route(?:s)?|port(?:s)?|facility|facilities)\b/i;
    ENERGY_EXPORT_PATTERN = /\b(?:crude|oil|petroleum)\s+exports?\b|\bexports?\s+of\s+(?:crude|oil|petroleum)\b/i;
    EVENT_RULES = [
      {
    ...
    normalizeText = (value2) => value2.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, " ").replace(/\s+/g, " ").trim();
    phraseMatches2 = (phrase, text2) => {
      const normalizedPhrase = normalizeText(phrase);
      const normalizedText = normalizeText(text2);
      if (!normalizedPhrase || !normalizedText) return false;
      return ` ${normalizedText} `.includes(` ${normalizedPhrase} `);
    };
    textValue = (value2) => typeof value2 === "string" ? value2.trim() : "";
    articleText = (article) => [
...
${timestamp}`;
    ...
    aliasesForCountry = (canonicalName, sourceNames) => [
      canonicalName,
      ...sourceNames,
      ...COUNTRY_ALIASES[canonicalName] || []
    ];
    buildCountryCandidates = (graph) => {
    ...
    findNodeMatches = (graph, text2) => graph.nodes.flatMap((node) => {
    ...
    findRegionMatches = (text2) => KNOWN_REGIONS.filter((region) => region.pattern.test(text2)).map((region) => ({ name: region.name, evidence: `known geographic signal matched "${region.name}".` }));
    findCountries = (graph, text2) => buildCountryCandidates(graph).filter((candidate) => candidate.aliases.some((alias) => phraseMatches2(alias, text2)));
    matchingRule = (text2) => {
    ...
    matchedEntityKey = (entity) => `${entity.entityType}:${entity.entityId || normalizeText(entity.name)}`;
    ...
    categoryForTradeRule = (text2) => /\b(?:sanction(?:s|ed)?|embargo)\b/i.test(text2) ? "sanctions" : "trade_restriction";
    locationFor = (nodeMatches, regionMatches, countries) => nodeMatches[0]?.node.name || regionMatches[0]?.name || countries[0]?.canonicalName;
    extractionResult = (event, confidence, matchedEntities, extractionReasons) => ({
      ...event ? { event } : {},
      confidence,
    ...
    extractDeterministicGeopoliticalEvent = (article, graph) => {
      if (!graph || !Array.isArray(graph.nodes) || !Array.isArray(graph.edges)) {
...
var import_node_crypto6, MonitoringRefreshConfigurationError, MonitoringRefreshTriggerError, DuplicateMonitoredEventError, IrrelevantMonitoringCandidateError, ExternalCandidateBudgetExceededError, DEFAULT_MONITORING_QUERIES, DEFAULT_POLL_INTERVAL_MS, MAX_POLL_INTERVAL_MS, DEFAULT_MAX_ARTICLES_PER_SCAN, MAX_ARTICLES_PER_SCAN, isTrue, positiveInteger, boundedArticleCount, envList, emptyScanSummary, scanResult, scanSummaryFromMeta, externalScanSummaryFromMeta, getMonitoringConfig, toMonitoringArticle, ENERGY_ARTICLE_TERMS, ENERGY_TRANSIT_TERMS, GEOPOLITICAL_ENERGY_TERMS, SUPPLY_THREAT_TERMS, isEnergyMonitoringCandidate, isEnergySupplyChainRelevant2, articleRequest, alertLevelFor, stableExternalArticleId, textField, nonNegativeIntegerField, createN8nMonitoringRefreshTrigger, ensureSchema, readMeta, writeMeta, cloneRecord, eventInputFor, articleInputFor, sourceReferenceFor, sourceReferencesFor, sourceAuthorityScore, referenceKey, latestIsoTimestamp, mergeArticleMetadata, eventFingerprintFor, eventMatches, rawArticleMatches, sourceLabel, GeopoliticalMonitoringService;
var init_monitoring = __esm({
  "src/geopoliticalEvents/monitoring.ts"() {
    ...
    init_groq();
    init_model2();
    init_deterministicExtractor();
    MonitoringRefreshConfigurationError = class extends Error {
      constructor() {
    ...
    MonitoringRefreshTriggerError = class extends Error {
      constructor(message) {
        super(message);
        this.name = "MonitoringRefreshTriggerError";
      }
    };
    DuplicateMonitoredEventError = class extends Error {
      constructor(articleId) {
        super(`Monitored event already exists: ${articleId}`);
        this.name = "DuplicateMonitoredEventError";
      }
    };
    IrrelevantMonitoringCandidateError = class extends Error {
      constructor() {
    ...
    ExternalCandidateBudgetExceededError = class extends Error {
    ...
    DEFAULT_MONITORING_QUERIES = ENERGY_MONITORING_QUERIES;
    DEFAULT_POLL_INTERVAL_MS = 15 * 60 * 1e3;
    MAX_POLL_INTERVAL_MS = 24 * 60 * 60 * 1e3;
    DEFAULT_MAX_ARTICLES_PER_SCAN = 5;
    MAX_ARTICLES_PER_SCAN = 100;
    isTrue = (value2) => value2?.trim().toLowerCase() === "true";
    positiveInteger = (value2, fallback) => Number.isInteger(value2) && value2 && value2 >= 1e4 && value2 <= MAX_POLL_INTERVAL_MS ? value2 : fallback;
    boundedArticleCount = (value2, fallback) => Number.isInteger(value2) && value2 && value2 >= 1 && value2 <= MAX_ARTICLES_PER_SCAN ? value2 : fallback;
    envList = (value2) => value2 ? value2.split(",").map((item) => item.trim()).filter(Boolean) : [];
    emptyScanSummary = (status) => ({
      status,
      articlesFetched: 0,
    ...
    scanResult = (retrievedAt, summary, eventsSkipped, alertsCreated) => ({
      ...summary,
      retrievedAt,
    ...
    externalScanSummaryFromMeta = (value2) => {
      if (!value2) return void 0;
      try {
        const parsed = JSON.parse(value2);
    ...
    ENERGY_ARTICLE_TERMS = /\b(?:energy|crude oil|oil|gas|natural gas|oil exports?|oil imports?|exports?|imports?|sanctions?|oil tanker|tankers?|refiner(?:y|ies)|pipelines?|oilfield|oil terminal|oil flows?|barrels?|petroleum|opec|lng|lpg|fuel shipment|export route|shipping lane)\b/i;
    ENERGY_TRANSIT_TERMS = /strait of hormuz|persian gulf|red sea|suez/i;
    GEOPOLITICAL_ENERGY_TERMS = /\b(?:iran|iraq|oman|saudi arabia|united arab emirates|uae|russia|yemen|qatar|kuwait|nigeria|venezuela)\b/i;
    SUPPLY_THREAT_TERMS = /\b(?:sanction(?:s|ed)?|embargo|disrupt(?:ion|ed)?|attack(?:ed)?|strike|conflict|war|tension|blockade|closure|outage|shutdown|seiz(?:e|ed|ure)|military|missile|drone|geopolitical|restriction|shortage|supply cut|production cut|reroute|avoid|alternative route|flows? stall(?:ed)?|halt(?:ed)?|hit|pirat(?:e|es)|fire|warning|alert|risk)\b/i;
    isEnergyMonitoringCandidate = (article) => {
    ...
    isEnergySupplyChainRelevant2 = (analysis) => analysis.classification.energyRelevant && analysis.relevance.relevant && analysis.risk.energyRelevant;
    articleRequest = (article) => JSON.stringify({
      title: article.title,
...
${publishedAt}`;
    ...
    textField = (value2, field, required = false) => {
    ...
    nonNegativeIntegerField = (value2, field) => {
      if (typeof value2 !== "number" || !Number.isInteger(value2) || value2 < 0) throw new Error(`${field} is invalid.`);
      return value2;
    };
    createN8nMonitoringRefreshTrigger = (fetchImplementation = fetch) => ({
  ...
  `);
    ...
    sourceAuthorityScore = (reference) => {
    ...
    latestIsoTimestamp = (values) => {
      const valid = values.filter((value2) => Boolean(value2 && !Number.isNaN(Date.parse(value2))));
      return valid.length ? valid.sort((left, right) => Date.parse(right) - Date.parse(left))[0] : values.find(Boolean);
    };
    mergeArticleMetadata = (existing, incoming) => {
    ...
    eventFingerprintFor = (article, analysis) => buildEventFingerprint(eventInputFor(article, analysis.event));
    eventMatches = (article, analysis, existing, storedEventFingerprint) => {
    ...
    rawArticleMatches = (article, existing) => {
    ...
    GeopoliticalMonitoringService = class {
      constructor(database2, agent, config = {}, source, refreshTrigger = createN8nMonitoringRefreshTrigger(), analysisOptions = {}) {
        this.database = database2;
        this.agent = agent;
...
var SCENARIO_SELECTABLE_NODE_TYPES, nodeTypeOrder, emptyTypeCounts, buildScenarioNodeList;
var init_scenario_node_catalog = __esm({
  "src/scenarios/scenario-node-catalog.ts"() {
    SCENARIO_SELECTABLE_NODE_TYPES = [
      "chokepoint",
      "port",
      "refinery",
      "shipping_route",
    ...
    buildScenarioNodeList = (graph, baselineProvider2) => {
      ...
      const nodes = graph.nodes.filter(
        ...
        (node) => baselineProvider2.getBaseline(
          {
...
// src/procurement/model.ts
var init_model3 = __esm({
  "src/procurement/model.ts"() {
  }
});

// src/procurement/solver-adapter.ts
var init_solver_adapter = __esm({
  "src/procurement/solver-adapter.ts"() {
  }
});

// src/procurement/glpk-solver-adapter.ts
var loadGlpk, toGlpkBounds, toGlpkModel, mapStatus, GlpkSolverAdapter;
var init_glpk_solver_adapter = __esm({
  "src/procurement/glpk-solver-adapter.ts"() {
    loadGlpk = async () => {
...
var DEFAULT_OBJECTIVE_WEIGHTS, isRecord4, isFiniteNumber, nonEmptyString, addIssue, validateProcurementRequest;
var init_input_validator = __esm({
  "src/procurement/input-validator.ts"() {
    ...
    isRecord4 = (value2) => typeof value2 === "object" && value2 !== null;
    isFiniteNumber = (value2) => typeof value2 === "number" && Number.isFinite(value2);
    nonEmptyString = (value2) => typeof value2 === "string" && value2.trim().length > 0;
    addIssue = (issues, path4, message) => {
      issues.push({ path: path4, message });
    };
    validateProcurementRequest = (input) => {
      const issues = [];
      if (!isRecord4(input)) {
        return {
          valid: false,
...
var variableNameForIndex, buildConstraint, buildProcurementOptimizationModel;
var init_optimization_model = __esm({
  "src/procurement/optimization-model.ts"() {
    variableNameForIndex = (index) => `procurement_${index}`;
    buildConstraint = (name, coefficients, upperBound, lowerBound) => ({
      name,
      coefficients,
      upperBound,
      lowerBound
    });
    buildProcurementOptimizationModel = (request) => {
...
var PROCUREMENT_VALIDATION_TOLERANCE, check, withinTolerance, validateProcurementAllocations;
var init_feasibility_validator = __esm({
  "src/procurement/feasibility-validator.ts"() {
    PROCUREMENT_VALIDATION_TOLERANCE = 1e-7;
    check = (constraint, passed, actual, limit, message) => ({
      constraint,
      passed,
      actual,
      limit,
    ...
    withinTolerance = (left, right) => Math.abs(left - right) <= PROCUREMENT_VALIDATION_TOLERANCE * Math.max(1, Math.abs(right));
    validateProcurementAllocations = (request, allocations) => {
      const checks = [];
...
var emptyValidation, zeroAllocations, buildSupplierAllocations, buildRouteAllocations, buildResult, invalidRequestResult, buildAllocations, ProcurementOrchestrator, optimizeProcurement;
var init_orchestrator = __esm({
  "src/procurement/orchestrator.ts"() {
    init_optimization_model();
    init_feasibility_validator();
    init_input_validator();
    init_glpk_solver_adapter();
    emptyValidation = () => ({
      valid: false,
      tolerance: PROCUREMENT_VALIDATION_TOLERANCE,
      checks: []
    });
    zeroAllocations = (request) => request.lanes.map((lane) => ({
      laneId: lane.laneId,
    ...
    buildSupplierAllocations = (request, allocations) => request.suppliers.map((supplier) => {
    ...
    buildRouteAllocations = (request, allocations) => request.routes.map((route) => {
    ...
    buildResult = (request, status, solverStatus, allocations, solveTimeMs, objectiveValue, constraintValidation, error) => {
    ...
    invalidRequestResult = (validationIssues) => ({
      status: "ERROR",
    ...
    buildAllocations = (request, laneVariableNames, variables) => request.lanes.map((lane) => {
    ...
    optimizeProcurement = async (request, solverAdapter) => new ProcurementOrchestrator(solverAdapter).optimize(request);
  }
});

// src/procurement/scenario-adapter.ts
var buildProcurementRequestFromScenario;
var init_scenario_adapter = __esm({
  "src/procurement/scenario-adapter.ts"() {
    buildProcurementRequestFromScenario = (scenario, graph, provider) => {
...
var DemoScenarioProcurementDataProvider;
var init_demo_scenario_provider = __esm({
  "src/procurement/demo-scenario-provider.ts"() {
    DemoScenarioProcurementDataProvider = class {
      resolve({ scenario }) {
...
var BARRELS_PER_METRIC_TONNE, REGIONAL_BENCHMARK_PROFILES, DEFAULT_BENCHMARK_PROFILE, EiaPriceService;
var init_eia_price_service = __esm({
  "src/procurement/eia-price-service.ts"() {
    BARRELS_PER_METRIC_TONNE = 7.33;
    REGIONAL_BENCHMARK_PROFILES = {
      "Saudi Arabia": {
...
var INDIAN_CRUDE_PORT_ANNUAL_CAPACITY_TONNES, CANONICAL_ROUTES, RealScenarioProcurementDataProvider;
var init_real_scenario_provider = __esm({
  "src/procurement/real-scenario-provider.ts"() {
    init_eia_price_service();
    INDIAN_CRUDE_PORT_ANNUAL_CAPACITY_TONNES = 24e7;
    CANONICAL_ROUTES = [
      {
    ...
    RealScenarioProcurementDataProvider = class {
...
var init_procurement = __esm({
  "src/procurement/index.ts"() {
    ...
    init_feasibility_validator();
    init_orchestrator();
    init_scenario_adapter();
    init_demo_scenario_provider();
    init_real_scenario_provider();
    init_eia_price_service();
  }
});

// src/reserves/model.ts
...
var import_express, import_node_crypto7, import_node_fs3, import_node_path3, import_node_process, import_vite, envLocalPath, queryText, queryInteger, queryBoolean, listOptions, dateQuery, handlePhase2Error, handleDigitalTwinError, handleGeopoliticalAgentError, handleMonitoringError, handleMonitoringRefreshError, parseScenarioInput, parseScenarioReserveAnalysis, readStateUpdate, stateSummary, createApp, isRunningTests;
var init_server = __esm({
  "server.ts"() {
    ...
    init_scenario_node_catalog();
    init_sqlite_baseline_provider();
    init_procurement();
    init_reserves();
    envLocalPath = import_node_path3.default.resolve(process.cwd(), ".env.local");
    ...
    queryInteger = (request, name, defaultValue, min = 1, max = 2200) => {
    ...
    parseScenarioReserveAnalysis = (body) => {
      ...
      for (const field of fields) {
        if (typeof value2[field] !== "number" || !Number.isFinite(value2[field]) || value2[field] < 0) {
          return {
    ...
    createApp = (repository2, digitalTwin = createDigitalTwinRuntime(repository2), geopoliticalRiskAgent = createGeopoliticalRiskIntelligenceAgent(
      digitalTwin,
      createGroqAgentProvider()
      ...
      const scenarioEngine2 = new ScenarioEngine(
      ...
      app.post(
        ...
        (request, response) => {
          ...
          try {
            ...
            const result = scenarioEngine2.run(
      ...
      app.post(
        ...
        async (request, response) => {
          ...
          try {
            ...
            const scenario = scenarioEngine2.run(digitalTwin.stateEngine, scenarioInput);
      ...
      app.post(
        ...
        async (request, response) => {
          ...
          try {
            const scenario = scenarioEngine2.run(
              digitalTwin.stateEngine,
              parsedInput.input
            );
            const resolution = buildProcurementRequestFromScenario(
      ...
      app.post(
        ...
        async (request, response) => {
          ...
          const durationDays = typeof disruptionDuration === "number" && disruptionDuration > 0 ? disruptionDuration : 30;
          try {
            ...
            const scenarioResult = {
              ...
              alternativeCapacityStatus: "VERIFIED",
    ...
    isRunningTests = process.env.NODE_ENV === "test" || process.argv.some((arg) => typeof arg === "string" && (arg.includes("test") || arg.includes("mocha") || arg.includes("jest")));
    if (!isRunningTests) {
...
// tests/phase8-end-to-end-validation.test.ts
...
var baselineProvider = new SqliteScenarioBaselineProvider(repository);
var scenarioEngine = new ScenarioEngine(baselineProvider);
(0, import_node_test.default)("Phase 8.4 End-to-End Validation: Full Flow Geopolitical Event -> Twin -> Supply Gap -> Real Alternative Procurement -> Reserve Optimizer", () => {
  ...
  import_strict.default.ok(saudiNode, "Must find Saudi Arabia node in real twin");
  ...
  const optimizerInput = {
    ...
    alternativeProcurement: Math.min(
      Math.round(dailyDisruptedTonnes * 0.4),
      // Let 40% be absorbed by alternative routes
...
(0, import_node_test.default)("Phase 8.4 Critical Safety Test Matrix (9 Required Test Scenarios)", () => {
  const reserveState = repository.getCurrentStrategicReserveState();
  const realProcurement = repository.getRealAlternativeProcurement();
  const testMatrix = [
    {
  ...
  for (const item of testMatrix) {
    ...
    import_strict.default.ok(passedSafety, `Failed safety constraint on test: ${item.name}`);
    console.log(`
Test Case: ${item.name}`);
    console.log("Inputs:", {
...
(0, import_node_test.default)("Phase 8.4 API Endpoint Validation (GET state, POST optimize, GET alternative-procurement, GET history)", async () => {
  const { createApp: createApp2 } = await Promise.resolve().then(() => (init_server(), server_exports));
  const { createServer } = await import("node:http");
  const app = createApp2(repository, runtime);
  ...
  try {
    ...
    import_strict.default.equal(stateRes.status, 200, "GET /api/reserves/state must return 200");
    const stateJson = await stateRes.json();
    import_strict.default.equal(stateJson.status, "AVAILABLE");
    import_strict.default.equal(stateJson.state.currentReserve, 5e6);
    import_strict.default.ok(stateJson.state.currentDemand > 0);
    import_strict.default.equal(stateJson.state.minimumReserveThreshold, 15e5);
    import_strict.default.ok(stateJson.state.alternativeProcurement);
    import_strict.default.equal(stateJson.state.alternativeProcurement.commercialCostStatus, "Commercial lane-cost data unavailable");
    const procRes = await fetch(`${baseUrl}/api/reserves/alternative-procurement`);
    import_strict.default.equal(procRes.status, 200, "GET /api/reserves/alternative-procurement must return 200");
    const procJson = await procRes.json();
    import_strict.default.equal(procJson.status, "AVAILABLE");
    import_strict.default.ok(procJson.procurement.suppliers.length > 0);
    import_strict.default.equal(procJson.procurement.commercialCostStatus, "Commercial lane-cost data unavailable");
    ...
    import_strict.default.equal(optRes.status, 200, "POST /api/reserves/optimize must return 200");
    const optJson = await optRes.json();
    import_strict.default.equal(optJson.status, "AVAILABLE");
    import_strict.default.equal(optJson.reserve.effectiveGap, 75e3);
    import_strict.default.equal(optJson.reserve.drawdownAmount, 225e4);
    import_strict.default.equal(optJson.reserve.remainingReserve, 275e4);
    import_strict.default.ok(optJson.reserve.remainingReserve >= optJson.reserve.minimumReserveConstraint);
    import_strict.default.equal(optJson.reserve.safetyConstraintGuaranteed, true);
    import_strict.default.ok(optJson.procurementProvenance);
    import_strict.default.equal(optJson.procurementProvenance.commercialCostStatus, "Commercial lane-cost data unavailable");
    const histRes = await fetch(`${baseUrl}/api/reserves/history?limit=5`);
    import_strict.default.equal(histRes.status, 200, "GET /api/reserves/history must return 200");
    const histJson = await histRes.json();
    import_strict.default.equal(histJson.status, "AVAILABLE");
    import_strict.default.ok(Array.isArray(histJson.runs));
    import_strict.default.ok(histJson.runs.length >= 1, "History must contain the run just executed");
    import_strict.default.equal(histJson.runs[0].result.drawdownAmount, 225e4);
  } finally {
    server.close();
  }
});
