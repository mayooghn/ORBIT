...
__export(optimizer_exports, {
  optimizeStrategicReserve: () => optimizeStrategicReserve,
  optimizeStrategicReserveWithProcurement: () => optimizeStrategicReserveWithProcurement,
  validateStrategicReserveInput: () => validateStrategicReserveInput
});
var INPUT_FIELDS, validateStrategicReserveInput, optimizeStrategicReserve, optimizeStrategicReserveWithProcurement;
var init_optimizer = __esm({
  "src/reserves/optimizer.ts"() {
    ...
    optimizeStrategicReserveWithProcurement = (input) => {
      const alternativeProcurement = typeof input.alternativeProcurement === "number" ? input.alternativeProcurement : input.procurementResult?.totalProcured ?? 0;
...
// tests/reserve-replacement-integration.test.ts
...
var validateGeopoliticalEvent = (value2) => {
  if (!isRecord2(value2)) throw new GeopoliticalEventValidationError("event must be an object.");
  return {
...
var validateClassification = (value2) => {
  if (!isRecord3(value2) || typeof value2.eventId !== "string" || typeof value2.energyRelevant !== "boolean") {
    throw new Error("A valid classified geopolitical event is required.");
  }
  return value2;
};
var validateRelevance = (value2) => {
  if (!isRecord3(value2) || typeof value2.eventId !== "string" || typeof value2.relevant !== "boolean") {
...
var validateRisk = (value2) => {
  if (!isRecord3(value2) || typeof value2.eventId !== "string" || !RISK_LEVELS.includes(value2.riskLevel) || typeof value2.riskScore !== "number" || !Number.isFinite(value2.riskScore) || value2.riskScore < 0 || value2.riskScore > 100 || typeof value2.energyRelevant !== "boolean" || !isStringArray(value2.matchedNodeIds)) {
...
var validateClassification2 = (event, value2) => {
  if (!isRecord4(value2)) throw new Error("A valid geopolitical event classification is required.");
...
var validateRelevance2 = (event, value2) => {
  if (!isRecord4(value2) || value2.eventId !== event.id || typeof value2.relevant !== "boolean") {
...
(0, import_node_test.before)(async () => {
  const app = createApp(repository, runtime, stubAgent, void 0, realDataProvider);
...
(0, import_node_test.default)("Integration: /api/pipeline/run with 30-day scenario confirms the Reserve Optimizer receives the correct inputs without procurement stage", async () => {
  ...
  import_strict.default.ok(body.assessment, "OrbitAssessment should be present");
  import_strict.default.equal(body.assessment.status, "COMPLETED");
  const reserveResult = body.assessment.reserve;
  import_strict.default.ok(reserveResult, "Reserve optimization should be present in assessment");
  import_strict.default.equal(reserveResult.input.disruptionDuration, durationDays);
  import_strict.default.equal(reserveResult.input.alternativeProcurement, 0);
  import_strict.default.equal(typeof reserveResult.input.supplyGap, "number");
...
(0, import_node_test.default)("Integration: /api/pipeline/run enforces alternativeProcurement = 0 per exact architecture specification", async () => {
  const manualOverride = 15e3;
  const response = await fetch(`${baseUrl}/api/pipeline/run`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
  ...
  import_strict.default.ok(reserveInput, "Reserve input should be present");
  import_strict.default.equal(reserveInput.alternativeProcurement, 0);
  console.log(`Verified exact specification: alternativeProcurement is 0.`);
});
(0, import_node_test.default)("Integration Case A: Partial external replacement", () => {
  const input = {
    currentReserve: 5e6,
    demand: 65e4,
    availableSupply: 3e5,
    // original gap = 350,000
    disruptionDuration: 10,
    alternativeProcurement: 1e5,
    // external replacement < 350,000
    replenishmentRate: 1e4,
    minimumReserveThreshold: 1e6
  };
  const { optimizeStrategicReserve: optimizeStrategicReserve2 } = (init_optimizer(), __toCommonJS(optimizer_exports));
  const result = optimizeStrategicReserve2(input);
  import_strict.default.equal(result.grossSupplyGap, 35e4, "Original supply gap should be 350k");
  import_strict.default.equal(result.residualSupplyGap, 25e4, "Remaining supply gap should be 250k");
  import_strict.default.equal(result.drawdownAmount, 25e5, "Reserve drawdown should handle only the remaining gap");
  console.log("Case A Verified: Partial external replacement correctly reduces remaining supply gap and drawdown.");
});
(0, import_node_test.default)("Integration Case B: Full external replacement", () => {
  const input = {
    currentReserve: 5e6,
    demand: 5e5,
    availableSupply: 4e5,
    // original gap = 100,000
    disruptionDuration: 10,
    alternativeProcurement: 15e4,
    // external replacement >= gap
    replenishmentRate: 1e4,
    minimumReserveThreshold: 1e6
  };
  const { optimizeStrategicReserve: optimizeStrategicReserve2 } = (init_optimizer(), __toCommonJS(optimizer_exports));
  const result = optimizeStrategicReserve2(input);
  import_strict.default.equal(result.grossSupplyGap, 1e5, "Original supply gap should be 100k");
  import_strict.default.equal(result.residualSupplyGap, 0, "Remaining gap should be 0");
  import_strict.default.equal(result.drawdownAmount, 0, "No unnecessary reserve release should be made");
  console.log("Case B Verified: Full external replacement results in 0 remaining gap and 0 drawdown.");
});
(0, import_node_test.default)("Integration Case C: No procurement result", () => {
  const input = {
    currentReserve: 5e6,
    demand: 5e5,
    availableSupply: 4e5,
  ...
  import_strict.default.equal(result.residualSupplyGap, 1e5);
  import_strict.default.equal(result.drawdownAmount, 1e6);
  console.log("Case C Verified: Reserve behavior with zero alternative procurement remains unchanged.");
});
(0, import_node_test.default)("Integration Case D: Procurement INFEASIBLE", () => {
  const input = {
    currentReserve: 5e6,
    demand: 5e5,
    availableSupply: 4e5,
    disruptionDuration: 10,
    alternativeProcurement: 0,
    // Infeasible results in 0 allocated capacity being committed
    replenishmentRate: 1e4,
    minimumReserveThreshold: 1e6
  };
  const { optimizeStrategicReserve: optimizeStrategicReserve2 } = (init_optimizer(), __toCommonJS(optimizer_exports));
...
(0, import_node_test.default)("Integration Case E: Manual alternativeProcurement", () => {
  const input = {
    currentReserve: 5e6,
    demand: 5e5,
    availableSupply: 4e5,
    disruptionDuration: 10,
    alternativeProcurement: 45e3,
    // Manual daily value
    replenishmentRate: 1e4,
    minimumReserveThreshold: 1e6
  };
  const { optimizeStrategicReserve: optimizeStrategicReserve2 } = (init_optimizer(), __toCommonJS(optimizer_exports));
  const result = optimizeStrategicReserve2(input);
  import_strict.default.equal(result.procurementCoverage, 45e3);
  import_strict.default.equal(result.residualSupplyGap, 55e3);
  console.log("Case E Verified: Manual alternative procurement remains unchanged.");
});
(0, import_node_test.default)("Integration Case F: Duration change and Single Multiplication Verification", () => {
  const duration = 20;
  const cumulativeProcurement = 6e5;
  const dailyEquivalent = cumulativeProcurement / duration;
  const input = {
    currentReserve: 5e6,
    demand: 5e5,
    availableSupply: 4e5,
  ...
  const result = optimizeStrategicReserve2(input);
  import_strict.default.equal(result.grossSupplyGap, 1e5);
  import_strict.default.equal(result.procurementCoverage, 3e4);
  import_strict.default.equal(result.residualSupplyGap, 7e4);
  import_strict.default.equal(result.drawdownAmount, 14e5, "Drawdown should be exactly Remaining Gap * Duration (70k * 20 = 1.4M)");
...
(0, import_node_test.default)("Integration: /api/procurement/optimize-gap with custom daily gap and duration", async () => {
  ...
  import_strict.default.equal(response.status, 200);
  const body = await response.json();
  import_strict.default.ok(body.status === "OPTIMAL" || body.status === "INFEASIBLE" || body.status === "UNAVAILABLE");
  console.log("Verified: custom daily gap and duration propagate and trigger optimization recalculation.");
});
(0, import_node_test.default)("Integration: /api/procurement/optimize-gap with custom dropdown source selection (Malacca)", async () => {
  const response = await fetch(`${baseUrl}/api/procurement/optimize-gap`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
...
(0, import_node_test.default)("Integration: /api/procurement/optimize-gap with invalid/empty inputs (invalid gap)", async () => {
  const response = await fetch(`${baseUrl}/api/procurement/optimize-gap`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
  ...
  import_strict.default.ok(body.error.includes("supplyGap"));
